        ...
        **or** if :py:`force=True`, :meth:`create` is called to regenerate a complete
        ...
        if existing is None or existing.version != cls.version or force:
            ...
            if isinstance(result, common.ItemScheme):
                # Ensure items have URNs
                for item in result:
                    if item.urn is None:
                        item.urn = sdmx.urn.make(item)
...
class ItemSchemeEnumType(EnumType):
    ...
    def __new__(metacls, cls, bases, dct, **kwargs) -> type["URNLookupEnum"]:
        ...
        # Add aliases
        for a, b in aliases:
            existing = enum_class.__dict__[a]
            if hasattr(existing, "_add_alias_"):
                # Python 3.13 (?) or later
                existing._add_alias_(b)
            else:
                setattr(enum_class, b, existing)
