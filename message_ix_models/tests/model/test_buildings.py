...
from message_ix_models.util import nodes_ex_world, package_data_path
...
def test_main_C_runs_with_materials(
    request: pytest.FixtureRequest,
    bmt_context_with_materials: "Context",  # noqa: F811
    tmp_path,
) -> None:
    """build_C runs with with_materials=True (materials linkage path)."""
    bmt_context_with_materials.buildings.method = METHOD.C
    bmt_context_with_materials.buildings.data_paths["sturm_r_ref"] = (
        tmp_path / "sturm_r.csv"
    )
    bmt_context_with_materials.buildings.data_paths["sturm_c_ref"] = (
        tmp_path / "sturm_c.csv"
    )
    scenario = bare_res(request, bmt_context_with_materials)
    _add_minimal_rc_pars(scenario)
    _add_materials_commodities(scenario)
...
def test_materials(request, bmt_context_with_materials, tmp_path):
    """materials runs and returns a dict with expected parametrization."""
    ...
    # simulate demand data as generated by MESSAGEix-Materials
    comms = ["aluminum", "steel", "cement"]
    dims = dict(level="demand", time="year", value=1.0, unit="-")
    s_info = ScenarioInfo(scenario)
    nodes = nodes_ex_world(s_info.N)
    df_mat_tot = make_df("demand", **dims, commodity=comms).pipe(
        broadcast, node=nodes, year=s_info.Y
    )
    with scenario.transact():
        scenario.add_set("level", "demand")
        scenario.add_par("demand", df_mat_tot)

    # construct artificial STURM material flow data
    # set material demand of STURM to 100% of total demand
    sturm_c_comms = [
        f"comm_mat_{flow}_{comm}" for comm, flow in product(comms, ["demand", "scrap"])
    ]
    sturm_r_comms = [
        f"resid_mat_{flow}_{comm}" for comm, flow in product(comms, ["demand", "scrap"])
    ]
    intensities = [i for i in BUILD_COMM_CONVERT if i.split("_")[-1] in comms]
    sturm_c = make_df(
        "demand",
        **dims,
        commodity=sturm_c_comms + [i for i in intensities if "comm" in i],
    ).pipe(broadcast, node=nodes, year=s_info.Y)
    sturm_r = make_df(
        "demand",
        **dims,
        commodity=sturm_r_comms + [i for i in intensities if "resid" in i],
    ).pipe(broadcast, node=nodes, year=s_info.Y)
    par_dict = materials(scenario, s_info, sturm_r, sturm_c)
