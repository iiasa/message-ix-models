...
def test_map_basin_region_wat(test_context):
    ...
    with open("supply_parity_results.txt", "a") as f:
        f.write(
            f"Time taken for map_basin_region_wat_new:"
    ...
    with open("supply_parity_results.txt", "a") as f:
        f.write(
            f"Time taken for map_basin_region_wat_legacy:"
            f" {(end_time - start_time) / n_iter} seconds\n"
        )

    # compare results
    pdt.assert_frame_equal(
        result_rf.sort_index(axis=0).sort_index(axis=1),
...
def test_add_water_supply_run(request, test_context):
    ...
    with open("supply_parity_results.txt", "a") as f:
        f.write(
            "Time taken for legacy add_water_supply: "
            f"{(end_time - start_time) / n_iter} seconds\n"
        )
    ...
    with open("supply_parity_results.txt", "a") as f:
        f.write(
            "Time taken for refactored add_water_supply: "
            f"{(end_time - start_time) / n_iter} seconds\n"
        )
    ...
    for key in ws_legacy:
        try:
            pdt.assert_frame_equal(
                ws_legacy[key].sort_index(axis=1), ws_refact[key].sort_index(axis=1)
            )
...
def test_add_e_flow(test_context):
    ...
    with open("supply_parity_results.txt", "a") as f:
        f.write(
            f"Time taken for new_add_e_flow: "
            f"{(end_time - start_time) / n_iter} seconds\n"
        )
    ...
    with open("supply_parity_results.txt", "a") as f:
        f.write(
            "Time taken for add_e_flow legacy: "
            f"{(end_time - start_time) / n_iter} seconds\n"
        )
    ...
    for key in result_rf:
        pdt.assert_frame_equal(
            result_rf[key].sort_index(axis=1), result[key].sort_index(axis=1)
        )
