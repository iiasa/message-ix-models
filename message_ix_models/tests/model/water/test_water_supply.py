...
from message_ix_models.model.water.data.water_supply import add_e_flow, add_water_supply, map_basin_region_wat
...
def test_map_basin_region_wat():
    ...
    with patch('message_ix_models.util.private_data_path', return_value="path/to/file"), \
        ...
        assert all(col in result.columns for col in ["region", "mode", "date", "MSGREG", "share", "year", "time"])
...
def test_add_water_supply():
    ...
    with patch('message_ix_models.util.private_data_path', return_value="path/to/file"), \
         ...
         patch('your_module.map_basin_region_wat', return_value=df_sw):
...
def test_add_e_flow():
    ...
    with patch('your_module.read_water_availability', return_value=(df_sw, df_sw)), \
         patch('message_ix_models.util.private_data_path', return_value="path/to/file"), \
         patch('pandas.read_csv', return_value=df_sw):
