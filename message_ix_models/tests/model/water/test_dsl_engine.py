...
def test_feed_make_df_invalid_type(dummy_rule):
    # Passing an invalid type to feed_make_df should raise a ValueError.
    with pytest.raises(ValueError):
        feed_make_df(dummy_rule, 123, skip_kwargs=[])




# Additional tests for _build_kw_args
...
def test_build_kw_args_node_loc_error_unexpected():
    # Should raise ValueError when node_loc is of an unexpected type.
    with pytest.raises(ValueError):
        _build_kw_args(
            flag_node_loc=True,
            node_loc=123,  # invalid type
            flag_time=False,
            sub_time=None,
            extra_args=None,
        )
