...
def test_config_file(tmp_path: Path) -> Path:
    """Write a test config file in an isolated temporary directory."""
    config_file = tmp_path / "config_test.yaml"
    with open(config_file, "w") as f:
        yaml.dump(_TEST_CONFIG_DATA, f, default_flow_style=False)
    return config_file
...
class TestConfig:
    ...
    def test_from_file(self, test_config_file: Path, name: str | None) -> None:
        """Configuration can be loaded from file."""
        path = test_config_file if name == "config_test" else name
...
def test_inter_pipe_bare_with_test_config(tmp_path, test_config_file, scenario) -> None:
    ...
    generate_bare_sheets(scenario, config_name=test_config_file, target_dir=tmp_path)
...
def test_build(tmp_path, test_config_file, scenario) -> None:
    ...
    build(scenario, config_name=test_config_file, data_dir=tmp_path)
