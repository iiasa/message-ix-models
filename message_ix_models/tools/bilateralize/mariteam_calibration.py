...
def calibrate_mariteam(
    covered_tec,
    message_regions,
    mtdict={
        "LNG_shipped": {
            "astd_ship_type": "Gas tankers",
            "flow_technology": ["LNG_tanker_LNG", "LNG_tanker_loil"],
        },
        "crudeoil_shipped": {
            "astd_ship_type": "Crude oil tankers",
        ...
        "eth_shipped": {
            "astd_ship_type": "Oil product tankers",
    ...
    out_path = os.path.join(
        os.path.dirname(package_data_path("bilateralize")), "bilateralize"
    )

    # Import MariTEAM outputs
    mtdf = pd.read_csv(os.path.join(mt_path, mt_output))
    mtdf = mtdf[
        mtdf[message_regions + "_origin"] != mtdf[message_regions + "_destination"]
    ].copy()  # no intraregional trade

    for tec in [i for i in covered_tec if "shipped" in i]:
        # If a non-energy commodity, add to bulk carriers in mtdict
        if tec.replace("_shipped", "") not in [
            "biomass",
            "coal",
            "crudeoil",
            "eth",
        ...
        for flow_fuel in mtdict[tec]["flow_technology"]:
            basedf = mtdf[
                mtdf["astd_ship_type"] == mtdict[tec]["astd_ship_type"]
            ].copy()
            ...
            mt_input["mt_value"] = (
                mt_input["intensity_MJ_tonne"] / mt_input["distance_km_sum"]
            )  # MJ/t-km
            ...
            mt_input = (
                mt_input.groupby(["node_loc", "technology", "unit"])["mt_value"]
                .sum()
                .reset_index()
            )

            regavg = (
                basedf.groupby(["node_loc"])[
                    ["energy_mj_sum", "dwt", "distance_km_sum"]
                ]
                .sum()
                .reset_index()
            )
            regavg["mt_value_reg"] = regavg["energy_mj_sum"] / (
                regavg["dwt"] * regavg["distance_km_sum"]
            )  # MJ/t-km
            regavg = regavg[["node_loc", "mt_value_reg"]]

            inputdf = pd.read_csv(
                os.path.join(
                    out_path, tec, "edit_files", "flow_technology", "input.csv"
                )
            )
            ...
            inputdf = inputdf.merge(
                regavg, left_on=["node_loc"], right_on=["node_loc"], how="left"
            )

            inputdf["value"] = np.where(
                inputdf["mt_value"] > 0, inputdf["mt_value"], inputdf["value"]
            )
            inputdf["value"] = np.where(
                (inputdf["value"].isnull())
                & (inputdf["technology"].str.contains(flow_fuel)),
                inputdf["mt_value_reg"],
                inputdf["value"],
            )
            ...
            if not os.path.isfile(
                os.path.join(
                    out_path, tec, "bare_files", "flow_technology", "input.csv"
                )
            ):
                inputdf.to_csv(
                    os.path.join(
        ...
        for flow_fuel in mtdict[tec]["flow_technology"]:
            ...
            basedf["mode"] = (
                basedf["R12_origin"].str.replace(message_regions + "_", "")
                + "-"
