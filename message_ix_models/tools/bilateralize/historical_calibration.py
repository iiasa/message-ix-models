...
def import_iea_balances(
    ...
    ieadf1 = pd.read_csv(
        os.path.join(
            data_paths["iea_web"], "WORLDBIG_by_COUNTRY_from_GIBRALTAR_to_RWANDA.txt"
        ),
        sep=r"\s+",
        header=None,
    )
    ieadf2 = pd.read_csv(
        os.path.join(
            data_paths["iea_web"],
            "WORLDBIG_by_COUNTRY_from_SAUDIARABIA_to_MAURITANIA.txt",
        ),
        sep=r"\s+",
        header=None,
    )
    ieadf3 = pd.read_csv(
        os.path.join(
            data_paths["iea_web"], "WORLDBIG_by_COUNTRY_from_WORLD_to_GHANA.txt"
        ),
        sep=r"\s+",
        header=None,
    )
...
def check_iea_balances(
    ...
    iea = (
        iea.groupby(["YEAR", "ISO3", "IEA-WEB COMMODITY", "IEA-WEB UNIT", "FLOW"])[
            "IEA-WEB VALUE"
        ]
        .sum()
        .reset_index()
    ...
    gas_direct = indf[
        indf["MESSAGE COMMODITY"].isin(["gas_piped", "LNG_shipped"])
    ].copy()
    ...
    for c in commodity_codes.keys():
        ...
        indf["COMMODITY"] = np.where(
            indf["MESSAGE COMMODITY"].str.contains(
                commodity_codes[c]["MESSAGE Commodity"]
            ),
            c,
            indf["COMMODITY"],
    ...
    iea = (
        iea.groupby(["YEAR", "ISO3", "COMMODITY", "IEA-WEB UNIT", "FLOW"])[
            "IEA-WEB VALUE"
        ]
        .sum()
        .reset_index()
    ...
    exports = exports[~exports["MULTIPLIER"].isnull()][
        ["YEAR", "EXPORTER", "COMMODITY", "MULTIPLIER"]
    ]
    basedf = basedf.merge(
        exports,
        left_on=["YEAR", "EXPORTER", "COMMODITY"],
        right_on=["YEAR", "EXPORTER", "COMMODITY"],
        how="left",
    )
    basedf["MULTIPLIER"] = np.where(
        basedf["MULTIPLIER"].isnull(), 1, basedf["MULTIPLIER"]
    )
...
def build_historical_activity(
    ...
    tradedf = check_iea_balances(
        indf=tradedf, project_name=project_name, config_name=config_name
    )
    ...
    if diagnosis_only:
        ...
        outdf = (
            outdf.groupby(["node_loc", "technology", "year_act", "mode", "time"])[
                "value"
            ]
            .sum()
            .reset_index()
