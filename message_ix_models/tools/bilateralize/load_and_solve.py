...
def remove_trade_tech(scen: message_ix.Scenario,
                      log,
                      config_tec: dict,
                      tec: str):
    """
    ...
    base_tec = [config_tec[tec][tec + '_trade']['trade_commodity'] + '_exp',
                config_tec[tec][tec + '_trade']['trade_commodity'] + '_imp',
                base_tec_name + '_exp',
                base_tec_name + '_imp',]
    base_tec = base_tec + [i for i in scen.set('technology') if
                config_tec[tec][tec + 'trade']['trade_commodity'] + '_exp_' in i]
...
def add_trade_sets(scen: message_ix.Scenario,
                   log,
                   trade_dict: dict,
                   tec: str):
    """
...
def add_trade_parameters(scen: message_ix.Scenario,
                         log,
                         trade_dict: dict,
                         tec: str):
    """
    Add parameters for bilateralized trade
    """
    new_parameter_list = list(set([i for i in
                              list(trade_dict[tec]['trade'].keys()) +
...
def update_relation_parameters(scen: message_ix.Scenario,
                               log,
                               trade_dict: dict,
                               tec: str):
    """
    Update relation parameters
    """
    rel_parameter_list = list(set([i for i in
                                        list(trade_dict[tec]['trade'].keys()) +
                                        list(trade_dict[tec]['flow'].keys())
                                        if 'relation_' in i]))
    ...
    if len(rel_parameter_list) > 0:
        for rel_par in ['relation_activity', 'relation_upper', 'relation_lower']:
            ...
            rel_par_list = list(set([i for i in
                                        list(trade_dict[tec]['trade'].keys()) +
                                        list(trade_dict[tec]['flow'].keys()) if
                                        rel_par in i]))

            for r in rel_par_list:
                if r in list(trade_dict[tec]['trade'].keys()):
                    rel_par_df = pd.concat([rel_par_df,
                                            trade_dict[tec]['trade'][r]])
                if r in list(trade_dict[tec]['flow'].keys()):
                    rel_par_df = pd.concat([rel_par_df,
                                            trade_dict[tec]['flow'][r]])
...
#%% Update bunker fuels
def update_bunker_fuels(scen: message_ix.Scenario,
                        tec: str,
                        log,
                        config_tec: dict):
    """
    ...
    if bunker_tec is not None:
        for btec in bunker_tec.keys():
            bunkerdf_in = scen.par('input',
                                    filters = {'technology': bunker_tec[btec]})
...
def update_additional_parameters(scen: message_ix.Scenario,
...
def remove_pao_coal_constraint(scen: message_ix.Scenario,
                               log,
...
def solve_or_save(mp: ixmp.Platform,
...
def load_and_solve(trade_dict,
    ...
    update_additional_parameters(scen = scen,
                      extra_parameter_updates = extra_parameter_updates)

    # Remove PAO coal and gas constraints on MESSAGEix-GLOBIOM
    remove_pao_coal_constraint(scen = scen,
                               log = log,
                               MESSAGEix_GLOBIOM = MESSAGEix_GLOBIOM)
