...
def full_broadcast(data_dict: dict,
                   tec: str,
                   ty: str,
                   ya_list: list[int],
                   yv_list: list[int],
    ...
    for i in data_dict[ty].keys():
            ...
            if "year_vtg" in data_dict[ty][i].columns and\
                "year_act" in data_dict[ty][i].columns:
                ...
                elif data_dict[ty][i]["year_vtg"].iloc[0] == "broadcast" and\
                        data_dict[ty][i]["year_act"].iloc[0] != "broadcast":
                    log.info(f"{i} in {tec} {ty} broadcasted for year_vtg")
                    data_dict[ty][i] = broadcast_years(df = data_dict[ty][i],
                                                        year_type = 'year_vtg',
                                                        year_list = yv_list)
            elif "year_vtg" in data_dict[ty][i].columns and\
                    "year_act" not in data_dict[ty][i].columns:
                if data_dict[ty][i]["year_vtg"].iloc[0] == "broadcast":
                    log.info(f"{i} in {tec} {ty} broadcasted for year_vtg")
                    data_dict[ty][i] = broadcast_years(df = data_dict[ty][i],
                                                        year_type = 'year_vtg',
                                                        year_list = yv_list)
            elif "year_vtg" not in data_dict[ty][i].columns and\
                    "year_act" in data_dict[ty][i].columns:
...
def build_parameter_sheets(log,
    ...
    for tec in covered_tec:
        ...
        # Broadcast the data
        for ty in ['trade', 'flow']:
            data_dict = full_broadcast(data_dict = data_dict,
                                       tec = tec,
                                       ty = ty,
                                       ya_list = ya_list,
                                       yv_list = yv_list,
