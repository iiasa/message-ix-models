...
def gem_region(project_name: str | None = None,
    ...
    full_path = package_data_path("bilateralize", "node_lists",
                                  message_regions + "_node_list.yaml")
    with open(full_path, "r") as f:
        message_regions = yaml.safe_load(f)
    message_regions_list = [r for r in message_regions.keys()\
                            if r not in ['World', 'GLB']]

    return message_regions_list, message_regions

# Import files
def import_gem(input_file: str | Path,
               input_sheet: str,
    ...
    for i in ['Start', 'End']:
        df = df.merge(cw,
                      left_on = i + 'Country', right_on = 'GEM Country',
                      how = 'left')
    ...
    for r in message_regions_list:
        df['EXPORTER'] = np.where(df['StartISO'].isin(message_regions[r]['child']),
                                  r,
                                  df['EXPORTER'])
        df['IMPORTER'] = np.where(df['EndISO'].isin(message_regions[r]['child']),
                                  r,
                                  df['IMPORTER'])

    # Collapse
    df['CapacityBOEd'] = df['CapacityBOEd'].replace('--', '0',
                                                    regex = True).astype(float)
    df['CostUSD'] = df['CostUSD'].replace('--', '0',
                                          regex = True).astype(float)
    df['LengthMergedKm'] = df['LengthMergedKm'].replace('--', '0',
                                                        regex = True).astype(float)

    df = df[(~df['CapacityBOEd'].isnull()) & (~df['CostUSD'].isnull())]

    df = df[df['StartYear1'] < first_model_year] # No planned capacity
    df['YEAR'] = round(df['StartYear1'].astype(float)/5)*5 # Round year to the nearest 5

    df = df.groupby(['EXPORTER', 'IMPORTER', 'YEAR'])\
                    [['CapacityBOEd', 'CostUSD', 'LengthMergedKm']].sum().reset_index()
    ...
    trade_dir = os.path.join(os.path.dirname(export_dir), trade_technology,
                             "edit_files")
    flow_dir = os.path.join(os.path.dirname(export_dir), trade_technology,
                            "edit_files", "flow_technology")
    trade_dir_out = os.path.join(os.path.dirname(export_dir), trade_technology,
                                 "bare_files")
    ...
    inv_cost = df.groupby(['EXPORTER', 'IMPORTER'])\
                          [['CostUSD','Capacity (GWa)']].sum().reset_index()
    ...
    inv_cost['technology'] = flow_technology + '_' +\
                             inv_cost['IMPORTER'].str.lower().str.split('_').str[-1]
    ...
    inv_cost = basedf.merge(inv_cost,
                            ...
                            how = 'left')
    inv_cost['value'] = np.where(inv_cost['value_update'] > 0,
                                 round(inv_cost['value_update'],0),
                                 inv_cost['value'])
    inv_cost['year_vtg'] = 'broadcast'
    inv_cost['unit'] = 'USD/km'
    inv_cost = inv_cost[['node_loc', 'technology',
                         'year_vtg', 'value', 'unit']].drop_duplicates()
    ...
    hist_act = df[['EXPORTER', 'IMPORTER',
                   'YEAR', 'LengthMergedKm']].drop_duplicates()
    hist_act['LengthMergedKm'] = np.where(hist_act['LengthMergedKm'].isnull(),
                                          0,
                                          hist_act['LengthMergedKm'])
    hist_act['LengthMergedKm'] = hist_act.groupby(['EXPORTER', 'IMPORTER'])\
                                                  ['LengthMergedKm'].transform(pd.Series.cumsum)
    hist_act['node_loc'] = hist_act['EXPORTER']
    hist_act['technology'] = flow_technology + '_' +\
                             hist_act['IMPORTER'].str.lower().str.split('_').str[-1]
    hist_act['value'] = round(hist_act['LengthMergedKm'],0)
    hist_act['year_act'] = hist_act['YEAR'].astype(int)
    hist_act['unit'] = 'km'
    hist_act['mode'] = 'M1'
    hist_act['time'] = 'year'
    hist_act = hist_act[['node_loc', 'technology', 'year_act',
                         'value', 'unit', 'mode', 'time']]
    hist_act.to_csv(os.path.join(export_dir, "historical_activity_GEM.csv"),
                    index = False)
    hist_act.to_csv(os.path.join(flow_dir, "historical_activity.csv"),
                    index = False)
    hist_act.to_csv(os.path.join(flow_dir_out, "historical_activity.csv"),
                    index = False)
    ...
    if trade_technology in ["crudeoil_piped", "loil_piped", "foil_piped"]:
        hist_tra = df[['EXPORTER', 'IMPORTER',
                       'YEAR', 'Capacity (GWa)']].drop_duplicates()
        hist_tra['Capacity (GWa)'] = np.where(hist_tra['Capacity (GWa)'].isnull(),
                                              0,
                                              hist_tra['Capacity (GWa)'])
        hist_tra['Capacity (GWa)'] = hist_tra.groupby(['EXPORTER', 'IMPORTER'])\
                                                      ['Capacity (GWa)'].transform(\
                                                                         pd.Series.cumsum)
        hist_tra['node_loc'] = hist_tra['EXPORTER']
        hist_tra['technology'] = trade_technology + '_exp_' +\
        ...
        hist_tra['mode'] = 'M1'
        hist_tra['time'] = 'year'
        hist_tra = hist_tra[['node_loc', 'technology', 'year_act',
                             'value', 'unit', 'mode', 'time']]
        hist_tra['year_act'] = hist_tra['year_act'].astype(int)
        hist_tra.to_csv(os.path.join(export_dir, "historical_activity_trade_GEM.csv"),
                        index = False)
        hist_tra.to_csv(os.path.join(trade_dir, "historical_activity.csv"),
                        index = False)
        hist_tra.to_csv(os.path.join(trade_dir_out, "historical_activity.csv"),
                        index = False)
    ...
    hist_cap = hist_cap.groupby(['EXPORTER', 'IMPORTER', 'YEAR'])\
                                ['CAPACITY_KM'].sum().reset_index()
    hist_cap = hist_cap.sort_values(by = ['EXPORTER', 'IMPORTER', 'YEAR'],
                                    ascending = True)
    hist_cap = hist_cap[(hist_cap['YEAR'] < first_model_year) &\
                        (hist_cap['YEAR'] > 1999)]
    ...
    hist_cap['technology'] = flow_technology + '_' +\
                             hist_cap['IMPORTER'].str.lower().str.split('_').str[-1]
    ...
    hist_cap = message_ix.make_df('historical_new_capacity',
                                  ...
                                  unit = 'km')
    hist_cap.to_csv(os.path.join(export_dir, "historical_new_capacity_GEM.csv"),
                    index = False)
    hist_cap.to_csv(os.path.join(flow_dir, "historical_new_capacity.csv"),
                    index = False)
    hist_cap.to_csv(os.path.join(flow_dir_out, "historical_new_capacity.csv"),
                    index = False)

    # Input
    inputdf = df.groupby(['EXPORTER', 'IMPORTER'])\
                         [['Capacity (GWa)', 'LengthMergedKm']].sum().reset_index()
    ...
    inputdf = basedf.merge(inputdf,
                           ...
                           how = 'left')
    inputdf['value'] = np.where((~inputdf['value_update'].isnull())&\
