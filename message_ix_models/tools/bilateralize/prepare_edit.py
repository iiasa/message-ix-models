...
def define_networks(log,
    ...
    node_path = package_data_path("bilateralize", "node_lists",
    ...
    for tec in covered_tec:
        ...
        if config_dict['trade_tech_suffix'][tec] is not None:
            node_df_tec['export_technology'] = node_df_tec['export_technology'] +\
                 '_' + config_dict['trade_tech_suffix'][tec]
        ...
        if config_dict['specify_network'][tec] is True:
            try:
                specify_network_tec = pd.read_csv(
                    os.path.join(data_path, tec,
                                 "specify_network_" + tec + ".csv"))
            except FileNotFoundError:
                node_df_tec.to_csv(os.path.join(data_path, tec,
                                                "specify_network_" + tec + ".csv"),
                                   index=False)
                raise Exception(
                    "The function stopped. Sheet specify_network_" +\
                    tec + ".csv has been generated. " +
...
def build_domestic_coalgas_relation(tec: str,
                                    network_setup: dict,
                                    config_dict: dict,
                                    common_years: dict,
                                    common_cols: dict,
...
def build_emission_factor(tec: str,
                          network_setup: dict,
                          config_dict: dict,
                          parameter_outputs: dict,
                          **kwargs):
...
def build_accounting_relations(tec: str,
                               network_setup: dict,
                               config_dict: dict,
                               common_years: dict,
                               common_cols: dict,
    ...
    # Regional exports
    rel_use = config_dict['trade_technology'][tec] + '_exp_from_' +\
    ...
    # Regional imports
    rel_use = config_dict['trade_technology'][tec] + '_imp_to_' +\
...
def build_flow_input(flow_tec: str,
                     tec: str,
                     network_setup: dict,
                     config_dict: dict,
                     common_years: dict,
   ...
   # List of commodity/material inputs
    ...
    for c in flow_inputs:
        ...
        input_out = pd.concat([parameter_outputs[tec]['flow']['input'],
                               df_input])
...
def build_flow_output(flow_tec: str,
                      tec: str,
                      network_setup: dict,
                      config_dict: dict,
                      common_years: dict,
    ...
    if config_dict['bunker_technology'][tec] is not None:
        ...
        df_output_gbunk['technology'] = 'bunker_global_' +\
            config_dict['flow_fuel_input'][tec][flow_tec][0]
    ...
    df_output['value'] = 1
    output_out = pd.concat([parameter_outputs[tec]['flow']['output'],
                            df_output])
...
def build_flow_capacity_constraints(par: str,
                                    flow_tec: str,
                                    tec: str,
                                    network_setup: dict,
                                    config_dict: dict,
    ...
    for t in ['lo', 'up']:
        ...
        con_out = pd.concat([parameter_outputs[tec]['flow'][par + '_' + t],
                              df_con])
...
def build_flow_FIcosts(flow_tec: str,
                       tec: str,
                       network_setup: dict,
                       config_dict: dict,
                       common_years: dict,
    ...
    for cost_par in ['fix_cost', 'inv_cost']:
        ...
        cost_out = pd.concat([parameter_outputs[tec]['flow'][cost_par],
                              df_cost])
        parameter_outputs[tec]['flow'][cost_par] = cost_out

    return parameter_outputs

def build_flow_Vcosts(flow_tec: str,
                      tec: str,
                      network_setup: dict,
                      config_dict: dict,
                      common_years: dict,
    ...
    vcost_out = pd.concat([parameter_outputs[tec]['flow']['var_cost'],
                            df_vcost_base])
...
def build_flow_capacity_factor(flow_tec: str,
                               tec: str,
                               network_setup: dict,
                               config_dict: dict,
                               common_years: dict,
    ...
    if config_dict['flow_constraint'][tec] == 'bilateral':
        df_cf_base['technology'] =  flow_tec + '_' +\
            df_cf_base['technology'].str.lower().str.split('_').str[-1]
    elif config_dict['flow_constraint'][tec] == 'global':
    ...
    if config_dict['bunker_technology'][tec] is not None:
        ...
        bdf = df_cf_base.copy()
        bdf['technology'] = 'bunker_global_' +\
            config_dict['flow_fuel_input'][tec][flow_tec][0]
        df_cf_base = pd.concat([df_cf_base, bdf])

    cf_out = pd.concat([parameter_outputs[tec]['flow']['capacity_factor'],
                        df_cf_base])
...
def build_flow_technical_lifetime(flow_tec: str,
                                  tec: str,
                                  network_setup: dict,
                                  config_dict: dict,
                                  common_years: dict,
    ...
    df_teclt_base =  build_parameterdf('technical_lifetime',
                                        network_df = network_setup[tec],
                                        col_values = dict(value = 20, # Default is 20y
                                                          unit = 'y'),
                                        export_only = True)

    if config_dict['flow_constraint'][tec] == 'bilateral':
        df_teclt_base['technology'] =  flow_tec + '_' +\
    ...
    teclt_out = pd.concat([parameter_outputs[tec]['flow']['technical_lifetime'],
                           df_teclt_base])
...
#%% Flow as trade input
def flow_as_trade_input(flow_tec: str,
                        tec: str,
                        network_setup: dict,
                        config_dict: dict,
                        common_years: dict,
    ...
    distance_df = pd.read_csv(os.path.join(data_path, "distances",
                                           message_regions + "_distances.csv"))
    ...
    mult_df['technology'] = config_dict['trade_technology'][tec] +\
        '_exp_' + mult_df['Node2'].str.lower().str.split('_').str[1]
    ...
    df_input_flow = df_input_flow.merge(mult_df,
    ...
    input_out = pd.concat([parameter_outputs[tec]['trade']['input'],
                           df_input_flow])
...
def export_edit_files(covered_tec: list[str],
    ...
    for tec in covered_tec:
        ...
        for parname in parameter_outputs[tec]['flow'].keys():
            parameter_outputs[tec]['flow'][parname].to_csv(
                os.path.join(data_path, tec, "edit_files", "flow_technology",
                             parname + ".csv"),
                index = False)
    ...
    for tec in covered_tec:
        reqpar_list = [os.path.join("capacity_factor.csv"),
        ...
        if 'shipping' in tec:
            ...
            for reqpar in required_parameters:
                if not os.path.isfile(os.path.join(data_path, tec,
                                                   "bare_files", reqpar)):
                    base_file = os.path.join(data_path, tec,
                                             "edit_files", reqpar)
#%% Main function to generate bare sheets
def generate_edit_files(log,
    ...
    config_base, config_path, config_tec = load_config(project_name,
                                                       config_name,
    ...
    # Generate full combination of nodes to build technology-specific network
    network_setup = define_networks(log,
                                    message_regions,
                                    covered_tec,
                                    config_dict,
                                    data_path)
...
def prepare_edit_files(project_name: str | None = None,
    ...
    for tec in [i for i in covered_tec if i != "gas_piped"]:
        ...
        input_df = input_df[['node_loc', 'technology', 'year_act',
                             'year_vtg', 'mode', 'time']].drop_duplicates()
        add_df = input_df.merge(add_df,
