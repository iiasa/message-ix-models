...
def bilat_trade_reporting(rep: Reporter,
                          scenario: message_ix.Scenario,
                          config_name: str,
                          out_dir:str = package_data_path('bilateralize', 'reporting', 'output', 'trade'),
                          energy_or_material: str = 'energy') -> pd.DataFrame:
    ...
    if energy_or_material == 'energy':
        df['Unit'] = 'EJ/yr'
        df['value'] = df['value'] * .03154
    elif energy_or_material == 'material':
        df['Unit'] = 'Mt/yr'
...
def trade_reporting(mp: ixmp.Platform,
                    scenario: message_ix.Scenario,
                    out_dir:str,
                    include_segments: list = ['primary_energy', 'secondary_energy', 'material']):
    ...
    secondarydf = pd.DataFrame()
    materialdf = pd.DataFrame()

    if 'primary_energy' in include_segments:
        primarydf = bilat_trade_reporting(rep, scenario, 'primary_energy_trade', out_dir, energy_or_material = 'energy')
    if 'secondary_energy' in include_segments:
        secondarydf = bilat_trade_reporting(rep, scenario, 'secondary_energy_trade', out_dir, energy_or_material = 'energy')
    if 'material' in include_segments:
        materialdf = bilat_trade_reporting(rep, scenario, 'material_trade', out_dir, energy_or_material = 'material')

    df = pd.concat([primarydf, secondarydf, materialdf])
