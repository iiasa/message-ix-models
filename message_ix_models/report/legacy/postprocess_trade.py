...
class PostProcess(object):
    ...
    def var_gdp(self, group=["Region"]):
        ...
        df = pp_utils._retr_var_gdp(self.ds).reset_index()
        return df.groupby(group).sum(numeric_only=True)

    def par_PPP(self, group=["Region"]):
        """Wrapper function to retrieve variable "GDP"(MER)
        ...
        df = pp_utils._retr_par_PPP(self.ds).reset_index()
        return df.groupby(group).sum(numeric_only=True)
    ...
    def out(self, tec = [], trade_tec=None,units=None, actfilter=None, outfilter=None, group=["Region"]):
        ...
        label = "ACT" if self.ix else "historical_activity"
        tec = [tec] if type(tec) == str else tec
        if trade_tec is not None:
            temp_tt = [i for i in self.ds.set("technology").unique() if any(i.startswith(tt) for tt in trade_tec)]
            if len(tec) == 0:
                tec = temp_tt
            else:
                tec = tec + temp_tt
    ...
    def emi(
        self, units, emifilter, emission_units, tec = [],trade_tec=None, actfilter=None, group=["Region"]
    ):
    ...
    def investment(self, units, tec = [], trade_tec=None,group=["Region"]):
    ...
    def tic_fom(self, units, tec = [], trade_tec=None, group=["Region"]):
        """Wrapper function to retrieve fixed O&M cost for a technology
    ...
    def act_vom(self, units, tec = [], trade_tec=None, actfilter=None, group=["Region"]):
