...
import message_ix_models.report.legacy.postprocess_trade as postprocess
...
def report(
    ...
    first_year : int
        Year as of which data should be updated.
    """
    nds0 = get_nodes(scen)
    ...
    if merge_ts:
        ...
        ts.Region = ts.Region.replace(reg_ts)
    ...
    for i in dfs:
        if merge_ts:
            ...
            tmp = ts[ts.Variable.str.find(var) >= 0].copy()
            tmp.Variable = tmp.Variable.str.replace(f"{var}|", "")
            if not tmp.empty:
                dfs[i] = (
    ...
    if merge_hist:
        ...
        if run_history == "True":
            years = [int(yr) for yr in col_yr]

        else:
            years = [int(yr) for yr in col_yr if yr >= scen.firstmodelyear]
        if first_year:
            years = [y for y in years if y >= first_year]

        cols = ["region", "variable", "unit"] + years

        ix_upload = ix_upload[cols]
        # mp._backend.jobj.unlockRunid(63433)
        ...
        df.Region = df.Region.replace(reg_ts)
        df = df.set_index(
    ...
    if not out_dir:
        out_dir = package_data_path("bilateralize", "reporting", "legacy")
    else:
