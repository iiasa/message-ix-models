...
from .postprocess import iamc_it, numcols, pp_utils, write_xlsx
from .util import (
...
def report(
    ...
    scenario_out: Optional[str] = None,
    out_dir: Optional[Path] = None,
    merge_hist: bool = False,
    merge_ts: bool = False,
    aggr_def: Optional[str] = None,
    var_def: Optional[str] = None,
    unit_yaml: Optional[str] = None,
    run_config: Optional[str] = None,
    urban_perc: Optional[str] = None,
    kyoto_hist: Optional[str] = None,
    lu_hist: Optional[str] = None,
    verbose: bool = False,
):
    ...
    with open(path / "default_run_config.yaml") as f:
        config = yload(f, Loader=SafeLoader)
    ...
    if run_config:
        with open(path / run_config) as f:
            tmp_config = yload(f, Loader=SafeLoader)
        for i in tmp_config:
    ...
    with open(unit_yaml) as f:
        data = yload(f, Loader=SafeLoader)
    # global mu
    mu = data["model_units"]
    for i in mu:
        try:
    ...
    for i in dfs:
        ...
        if run_tables[i]["root"] == "Emissions|HFC":
            ...
            df_list.append(
                iamc_it(df=dfs[i], what=run_tables[i]["root"], mapping=mapping)
            )
    ...
    if merge_hist:
        ...
        if run_history == "True":
            ...
            cols = ["region", "variable", "unit"] + [
                int(yr) for yr in col_yr if int(yr) >= model_year
            ]
    ...
    if not out_dir.exists():
        out_dir.mkdir()
    write_xlsx(df, out_dir, model_name=pp_utils.model_nm, scen_name=pp_utils.scen_nm)
