"""Command-line tools specific to adding CoC (Cost of Capital) to the MESSAGE model."""
...
# Define a command-line option for coc scenarios
_SCENARIO = click.Option(
    ["--scenario", "coc_scenario"],
    default="baseline",
    help="The scenario ID of the specific step in the coc workflow.",
)


# Define a Click command group for coc-related commands
@click.group("coc", params=[_SCENARIO])
@click.pass_obj
def cli(context, coc_scenario):
    """Runs with cost of capital in MESSAGE."""
    # Store the scenario in the context for use in commands
    context.coc = coc_scenario
    pass
...
def run(context, truncate_step, target_step):
    """Run the coc workflow up to step TARGET.
    ...
    if context.dry_run:
        path = context.get_local_path("coc-workflow.svg")
