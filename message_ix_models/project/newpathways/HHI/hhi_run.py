...
for region in ['WEU']:
    ...
    hhi_tec_nodes = hhi_config['hhi_R12_' + region]
    
    for limit in hhi_limits:
        print('... HHI limit = ' + str(limit))
        
        # Clone scenario
        scen = base.clone(model='NP_SSP2_6.2', scenario='HHI_' + str(limit) + '_gas_' + region, keep_solution = False)
        print('Scenario cloned: ' + 'HHI_' + str(limit) + '_gas_' + region)
        scen.set_as_default()
    
        # Add HHI limit
        add_HHI(scen=scen,
                hhi_tec_nodes=hhi_tec_nodes,
                hhi_portfolio_node="R12_" + region,
                hhi_limit=limit)
