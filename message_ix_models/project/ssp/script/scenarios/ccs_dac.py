...
def gen_te_projections(
    ...
    ssp: str
        SSP to use for projection assumptions. Must be one of "all", "LED", "SSP1",
        "SSP2", "SSP3", "SSP4", "SSP5".
    method: str
    ...
    """
    assert ssp in "all LED SSP1 SSP2 SSP3 SSP4 SSP5".split()
    ...
    cfg = Config(
        ...
        method=method,
        format="message",
        scenario=ssp,  # type: ignore [arg-type]
    )
...
for ssp in ssps:
    ...
    filepath = (
        r"C:\Users\pratama\Documents\GitHub\MESSAGEix\message-ix-models"
        r"\message_ix_models\data\ccs-dac"
    )
    add_tech(scen, filepath + rf"\co2infrastructure_data_{ssp}dev.yaml")
    ...
    add_tech(scen, filepath + rf"\daccs_setup_data_{ssp}dev.yaml")
