...
logging.info("Logging configuration set up.")  # Log to ensure configuration is applied
...
# Call log_memory_usage to test
log_memory_usage()

# just for solving locally
# scenarioName = "NPi2020"
# scen2Name = "NPi2020_GDP_CI"
# sc = sc_ref.clone(modelName, scen2Name, keep_solution=False)

# solve
# sc.solve(solve_options={"lpmethod": "4"},model=run_mode)
#
...
for scenario in scens_ref:
    ...
    # only run the initial scenario, if magic file not existing
...
for scenario in scens_ref:
    # initiate scenario
    sc0 = message_ix.Scenario(
        mp, model_name_clone, scenario + "_GDP_CI_" + str(0), cache=True
    )
    run_legacy_reporting(sc0, mp)
#### end of actual build bloc
