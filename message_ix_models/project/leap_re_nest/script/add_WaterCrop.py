...
def add_irri_demand(sc,ss):
    ...
    try:
        ...
        print('No urban water demand existing, '
    ...
    map_ba = pd.DataFrame({"node": ba,
                           "BCU" : list(range(1,25))})
    # shape output file
    dem_out = (dem_df.merge(map_ba, how = "left")
               .assign(commodity="irr_water_basin")
               )
...
def irri_tecs(sc):
    ...
    if len(cl) == 0:
        print("ERROR: Your demand input file does not contain crops for the scenario",
              sc.scenario)
        return
    
    for cr in cl:
        print("Adding crop: ", cr)
        irr_tec = crop_par_csv.loc[
            crop_par_csv['crop'] == cr, 'irr_type'].values[0]
        irr_eff = crop_par_csv.loc[
            crop_par_csv['crop'] == cr, 'eta_irr'].values[0]
        tec_name = "irr_" + cr + "_" + irr_tec
        sc.add_set("technology", tec_name)
        lvl = "irr_" + cr
        # lifetime of offgrid technologies
        lt = irr_par_csv.loc[
            irr_par_csv['irr_type'] == irr_tec, 'lifetime'].values[0]
        yv_ya_gr = map_yv_ya_lt(years[years >= first_year-lt], lt, first_year)
        # covert from kWh/m3 to GWa/km3
        en_inp = irr_par_csv.loc[
            irr_par_csv['irr_type'] == irr_tec, 
            'en_inp_kWh_m3'].values[0] * 1000 / 8760
        # costs still in USD/ha
        inv_cost = irr_par_csv.loc[
            irr_par_csv['irr_type'] == irr_tec, 'inv_USD_ha'].values[0]
        var_cost = irr_par_csv.loc[
            irr_par_csv['irr_type'] == irr_tec, 'var_USD_ha'].values[0]
        fix_cost = irr_par_csv.loc[
            irr_par_csv['irr_type'] == irr_tec, 'fix_USD_ha'].values[0]
        irr_inp = pd.concat([irr_inp,
                             (make_df(
                                 "input",
                                 technology= tec_name,
                                 value=1,
