...
def update_res_marg(sc, path_xls, sc_ref=[], # based on a reference scenario
                    ...
                    xls_sheet='peak_demand', nodes='all'):
    ...
    resm_old = sc.par(parname, {'relation': 'res_marg',
                                'technology': 'elec_t_d'})
    if sc_ref:
        resm_ref = sc_ref.par('relation_activity', {'relation': 'res_marg',
                                        'technology': 'elec_t_d'})
    ...
    if 'time' not in sc.idx_names(parname):
        ...
        if sc_ref:
            resm_ref['time'] = 'year'
            resm_ref = resm_ref.set_index(['node_loc', 'year_act', 'time']
                                          ).sort_index()
    ...
    d = dem.pivot_table(values='value',
                           columns='year', index=['node', 'time'])
    dem_growth = d.copy()
    ...
    for t in set(sc.set('time')):
        dem.loc[dem['time'] == t, 'actual'
                ] = dem.loc[dem['time'] == t, 'value'] / float(
            dur.loc[dur['time'] == t, 'value'])
    ...
    for i in resm.index:
        ...
        if (peak_dem / max_dem) < 1:
            new_res = - (1. + reserve_margin)
        else:
            new_res = - (peak_dem / max_dem) * (1. + reserve_margin)
...
def historical_to_ref(sc, year_list=[], sc_ref=None, regions=all,
    ...
    par_list = ['historical_activity', 'historical_new_capacity',
    ...
    for parname in par_list:
        ...
        if not df_par.empty:
            ...
            print('> The content of parameter "{}" was copied to "{}" to the scenario.'.format(parname,
...
if __name__ == "__main__":
    ...
    com_div = set(com_div)
    print(
        "- Divider technologies being added for commodity/levels of"
        ": {}".format(com_div)
    )
    ...
    print(
        "- Collector technologies being added for commodity/levels of"
        ": {}".format(com_coll)
    )
