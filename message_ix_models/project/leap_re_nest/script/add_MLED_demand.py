...
def main(sc, ss):
    ...
    dem_el_old = sc.par("demand", {"year": firstmy, "commodity": ["i_spec", "rc_spec"]})
    ...
    sc.add_set(
        "commodity",
        ["ind_man_urb", "ind_man_rur", "res_com_urb", "res_com_rur", "crop_rur"],
    )
    sc.add_par("demand", dem)
    sc.commit("")
    print("Electricity demand from MLED added " + str(tot_dem_el_new) + " GWa/a")
