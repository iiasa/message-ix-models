"""Command-line tools specific to ELEVATE GP scenarios."""
...
# Define a command-line option for ELEVATE GP scenarios
_SCENARIO = click.Option(
    ["--scenario", "elevate_gp_scenarios"],
    default="baseline",
    help="The scenario ID of the specific step in the elevate gp workflow.",
)


# Define a Click command group for ELEVATE GP-related commands
@click.group("elevate", params=[_SCENARIO])
@click.pass_obj
def cli(context, elevate_gp_scenarios):
    """MESSAGEix ELEVATE GP scenarios."""
    # Store the scenario in the context for use in commands
    context.elevate_gp = elevate_gp_scenarios
    pass
...
def run(context, truncate_step, target_step):
    """Run the ELEVATE GP workflow up to step TARGET.
    ...
    if context.dry_run:
        path = context.get_local_path("elevate_gp_workflow.svg")
