"""Command-line tools specific to EFC scenarios."""
...
# Define a command-line option for EFC scenarios
_SCENARIO = click.Option(
    ["--scenario", "efc_scenario"],
    default="baseline",
    help="The scenario ID of the specific step in the EFC workflow.",
)


# Define a Click command group for EFC-related commands
@click.group("efc", params=[_SCENARIO])
@click.pass_obj
def cli(context, efc_scenario):
    """MESSAGEix EFC scenarios."""
    # Store the scenario in the context for use in commands
    context.efc_scenario = efc_scenario
    pass
...
def run(context, truncate_step, target_step):
    """Run the EFC workflow up to step TARGET.
    ...
    if context.dry_run:
        path = context.get_local_path("efc_workflow.svg")
