"""Command-line tools specific to NGFS scenarios."""
...
# Define a command-line option for NGFS scenarios
_SCENARIO = click.Option(
    ["--scenario", "ngfs_scenarios"],
    default="baseline",
    help="The scenario ID of the specific step in the ngfs workflow.",
)


# Define a Click command group for NGFS-related commands
@click.group("ngfs", params=[_SCENARIO])
@click.pass_obj
def cli(context, ngfs_scenarios):
    """MESSAGEix NGFS scenarios."""
    # Store the scenario in the context for use in commands
    context.ngfs = ngfs_scenarios
    pass
...
def run(context, truncate_step, target_step):
    """Run the NGFS workflow up to step TARGET.
    ...
    if context.dry_run:
        path = context.get_local_path("ngfs-workflow.svg")
