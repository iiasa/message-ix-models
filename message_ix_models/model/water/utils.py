...
def ssp2_rate_files(path: Path) -> list[Path]:
    """Paths to the ssp2 treatment- and recycling-rate CSVs that exist under *path*.

    Missing files are omitted so that callers see the same "no CSVs found"
    condition they would get from a glob, rather than a :class:`FileNotFoundError`
    when reading.
    """
    candidates = (path / f"ssp2_regional_{rate}_baseline.csv" for rate in SSP2_RATES)
