...
def _build_pipe_arguments(
    ...
    lt_arg: str | None
        The argument to pass to the lifetime if it is a dataframe/series.
    year_wat: tuple | None
    ...
    match (flag_map_yv_ya_lt, lt):
        ...
        case True, _:
            raise TypeError(
                f"Expected integer for 'lt' when flag_map_yv_ya_lt is True, "
                f"got {type(lt)}"
            )
...
def _build_kw_args(
    ...
    match (flag_node_loc, node_loc):
        ...
        case (True, None):
            raise ValueError(
                "node_loc if flag_node_loc is True"
            )
        case (True, _):
