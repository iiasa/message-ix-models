...
# TODO: set up path in a better way within the nexus/water module
...
for cl in climmodels:
    ...
    for scen in scenarios:
        ...
        quant = 0.1  # NOTE: what is this for? not used anywhere else
        # define if multi climate models mean
        multimodelensemble = True  # NOTE: what is this for? not used anywhere else
        ...
        area = xr.open_dataarray(os.path.join(wd1, "landareamaskmap0.nc"))
        ...
        if monthlyscale:
            years = np.arange(2010, 2105, 5)  # NOTE: same for true and false
        else:
        ...
        if monthlyscale:
            ...
            if var == "dis":
                ...
                davg.to_netcdf(wd2 + f"{var}_90Y_avg_5y__{cl}_{scen}_temp_agg.nc")
                ...
                da.to_netcdf(wd2 + f"{var}_5y__{cl}_{scen}_temp_agg.nc")
                # read in saved data again to further process
                da = xr.open_dataset(wd2 + f"{var}_5y__{cl}_{scen}_temp_agg.nc")
                ...
                da = da.resample(time="5Y", loffset="4Y").mean()
                da.to_netcdf(wd2 + f"{var}_monthly__{cl}_{scen}_temp_agg.nc")
                # read in saved data again to further process
                # da = xr.\
                ...
                da.to_netcdf(wd2 + f"{var}_5y__{cl}_{scen}.nc")

        print("....Completed " + cl + " + " + scen)
