...
map_iso3 <- data.frame(
  country = unique(desal_data.df$country),
...
map_iso3 <- data.frame(
  country = unique(desaldata.df$country),
...
desal_new_cap.df <- desal_data.df %>%
  rename(year = online) %>%
  group_by(iso3, year) %>%
  summarise(desal_cap = sum(m3_per_day)) %>%
  ungroup() %>%
  group_by(iso3) %>%
  mutate(cum_desal = cumsum(desal_cap)) %>%
  ungroup()

# from desalCAPACITYDATA database
desal_new_cap2.df <- desaldata.df %>%
  rename(year = online) %>%
  filter(!is.na(year), !is.na(m3_per_day)) %>%
  group_by(iso3, year) %>%
  summarise(desal_cap = sum(m3_per_day)) %>%
  ungroup() %>%
  group_by(iso3) %>%
...
desal_new_cap_glb.df <- desal_new_cap2.df %>%
  group_by(year) %>%
  summarise(
    desal_cap = sum(desal_cap),
    cum_desal = sum(cum_desal)
  ) %>%
  ungroup() %>%
...
desal_comp <- desal_new_cap.df %>%
  ...
  group_by(iso3) %>%
  mutate(
    max_cumacp1 = max(cum_desal),
    max_cumcap2 = max(cumsum2)
  ) %>%
  ungroup() %>%
  filter(
    (cum_desal == max_cumacp1),
    cum_desal > 1000000
  )
...
gdp <- read.csv("P:/ene.model/NEST/GDP/WB_GDP_2021/GDP_country_cleaned.csv") %>%
  gather(key = "year", value = "gdp", 5:65) %>%
...
gdp_new <- gdp_new_hist_proj %>%
  select(iso3c, year, gdppc) %>%
  rename(iso3 = iso3c, gdp = gdppc) %>%
  drop_na()
...
wgi <- read.dta13("P:/ene.model/NEST/governance/wgidataset.dta") %>%
  ...
  spread(var, value) %>%
  mutate(governance = rowMeans(select(
    ., voic.ac,
    pol.stab, gov.eff, reg.qual, ru.law, corr.cont
  ))) %>% # # Governance variable as the arithmetic average of the six components of WGI) %>%
  select(-countryname)
...
wsi.df <- wsi_hist %>%
  gather(key = year, value = wsi, 3:length(wsi_hist)) %>%
...
master <- desal_new_cap3.df %>%
  ...
  group_by(iso3) %>%
  mutate(
    wsi = as.numeric(if_else(is.na(wsi), stats::quantile(wsi, 0.75, na.rm = T), wsi)),
...
adjust_fe <- function(fe) {
  ...
  conv.rates <- data.frame(
    years = c(110, 230, 3000, 230, 160),
...
ggplot(projected %>% filter(
  iso3 %in% quantiles[quantiles$quant == "q2_3", ]$iso3,
  !iso3 %in% c("USA", "ARE", "SAU", "ESP", "KWT", "QAT")
)) +
  geom_line(aes(x = year, y = exp(log_desal)), color = "black") +
  # geom_line(aes(x = year, y = exp(log_desal2)), color = "blue") +
...
for (rcp in RCPs) {
  ...
  hist_cum_cap <- master %>%
    select(iso3, year, log_desal) %>%
    group_by(iso3) %>%
    mutate(hist_cap = max(log_desal)) %>%
    ungroup() %>%
    select(iso3, hist_cap) %>%
    distinct()
  ...
  fut_proj_basin <- future_proj_out %>%
    group_by(BCU_name, year) %>%
    summarise(cap_m3_day = mean(desal_cap)) %>%
    ungroup() %>%
...
comp_glob <- comp %>%
  group_by(rcp, year) %>%
  summarise(cap_km3_year = sum(cap_km3_year)) %>%
  ungroup()
...
hist_cap.df <- read.csv("P:/ene.model/NEST/desalination/DESALCAPACITYDATA2016_current_online.csv") %>%
  mutate(tec_type = if_else(technology %in% c(
  ...
  summarise(cap_km3_year = sum(m3_per_day) * 1e-9 * 365) %>%
  ungroup() %>%
  drop_na()

map_hist_years <- rbind(
...
hist_cap.df <- hist_cap.df %>%
  left_join(map_hist_years %>% rename(year = sy)) %>%
...
high2020 <- hist_comp %>%
  filter(year == 2015, diff2020 < 0) %>%
  mutate(
    Xhist = hist_cap_cum / cap_km3_year,
