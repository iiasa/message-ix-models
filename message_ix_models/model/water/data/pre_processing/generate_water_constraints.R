...
dat.df <- merge_recurse(lapply(seq(2010, 2090, by = 10), function(y) {
  ...
  dat.df <- cbind(
    dat.df[, c("country_id", paste("xloc", y, sep = "."), paste("yloc", y, sep = "."), paste("urban_pop", y, sep = "."), paste("rural_pop", y, sep = "."), paste("urban_gdp", y, sep = "."), paste("rural_gdp", y, sep = "."))],
    rowSums(do.call(cbind, lapply(1:12, function(m) {
      dat.df[, paste("urban_withdrawal", y, m, sep = ".")]
    }))),
    rowSums(do.call(cbind, lapply(1:12, function(m) {
      dat.df[, paste("rural_withdrawal", y, m, sep = ".")]
    }))),
    rowSums(do.call(cbind, lapply(1:12, function(m) {
      dat.df[, paste("urban_return", y, m, sep = ".")]
    }))),
    rowSums(do.call(cbind, lapply(1:12, function(m) {
      dat.df[, paste("rural_return", y, m, sep = ".")]
    }))),
    rowSums(do.call(cbind, lapply(1:12, function(m) {
      dat.df[, paste("urban_withdrawal", y, m, sep = ".")]
    }))) / dat.df[, paste("urban_pop", y, sep = ".")],
    rowSums(do.call(cbind, lapply(1:12, function(m) {
      dat.df[, paste("rural_withdrawal", y, m, sep = ".")]
    }))) / dat.df[, paste("rural_pop", y, sep = ".")]
  )
  ...
  if (length(inds) > 0) {
    dat.df[inds, paste("urban_return", y, sep = ".")] <- dat.df[inds, paste("urban_withdrawal", y, sep = ".")] * 0.92
  }
  inds <- which(unlist(dat.df[, paste("rural_return", y, sep = ".")]) > 0.92 * unlist(dat.df[, paste("rural_withdrawal", y, sep = ".")]))
  if (length(inds) > 0) {
    dat.df[inds, paste("rural_return", y, sep = ".")] <- dat.df[inds, paste("rural_withdrawal", y, sep = ".")] * 0.92
  }
...
dat.df$country <- sapply(dat.df$country_id, function(cc) {
  if (cc %in% country_region_map_key.df$UN_Code) {
    return(country_region_map_key.df$ISO3[which(country_region_map_key.df$UN_Code == cc)])
  } else {
    return(NA)
  }
})
dat.df$R11 <- sapply(dat.df$country_id, function(cc) {
  if (cc %in% country_region_map_key.df$UN_Code) {
...
dat.df$R12 <- sapply(dat.df$country_id, function(cc) {
  if (cc %in% country_region_map_key.df$UN_Code) {
...
presence_of_coast <- dat.spdf@data %>%
  filter(distance2coast <= 1.5) %>%
  mutate(num = as.numeric(1)) %>%
  select(country, distance2coast, num) %>%
  group_by(country) %>%
...
ww.df2_plot <- ww.df2 %>%
  group_by(country_id) %>%
  summarise(
    income.2020 = mean(income.2020),
    udw_raw = mean(udw_raw),
    usa_raw = mean(usa_raw)
  ) %>%
  ungroup() %>%
...
ww_proj <- function(ww.df2, UR, yy) {
  ...
  ww.df2 %>%
    ...
    mutate(
      !!varc := pmin(0.999, if_else(year == 2010, 0.95 * c0,
        (1 + (c0 / c_mod - 1) *
...
ww_plot <- dat.spdf[c(
  "country", "urban_pop.2020", "rural_pop.2020", "urban_connection_rate.2020",
...
totals_ww <- ww_plot@data %>% summarise(
  urban_no_wa.2020 = sum(urban_no_wa.2020),
...
if (scn %in% c("sdg6")) {
  ...
  dat.df <- cbind(dat.df, do.call(cbind, lapply(yrs, function(yy) {
    ...
    mfpc <- unlist(sapply(cnt, function(cc) {
      return(mf_withdrawal.df[which(as.character(mf_withdrawal.df$Scenario) == fsc & as.character(mf_withdrawal.df$Country_Code) == cc), paste("X", yy, sep = "")] / sum(dat.df[which(dat.df$country == cc), paste("urban_pop", yy, sep = ".")], na.rm = TRUE))
    }))
    tmp.df <- data.frame(country = dat.df$country)
    tmp.df$res <- NA
    for (x in cnt) {
      tmp.df$res[as.character(tmp.df$country) == x] <- dat.df[as.character(dat.df$country) == x, paste("urban_pop", yy, sep = ".")] * mfpc[x]
    }
  ...
  dat.df <- cbind(dat.df, do.call(cbind, lapply(yrs, function(yy) {
    ...
    mfpc <- unlist(sapply(cnt, function(cc) {
      return(mf_return.df[which(as.character(mf_return.df$Scenario) == fsc & as.character(mf_return.df$Country_Code) == cc), paste("X", yy, sep = "")] / sum(dat.df[which(dat.df$country == cc), paste("urban_pop", yy, sep = ".")], na.rm = TRUE))
    }))
    tmp.df <- data.frame(country = dat.df$country)
    tmp.df$res <- NA
    for (x in cnt) {
  ...
  for (yyy in yrs[yrs >= 2030])
  {
    ...
    if (length(inds) > 0) {
      dat.df[inds, paste("urban_return", yyy, sep = ".")] <- dat.df[inds, paste("urban_withdrawal", yyy, sep = ".")] * 0.92
    }
    inds <- which(unlist(dat.df[, paste("rural_return", yyy, sep = ".")]) > 0.92 * unlist(dat.df[, paste("rural_withdrawal", yyy, sep = ".")]))
    if (length(inds) > 0) {
      dat.df[inds, paste("rural_return", yyy, sep = ".")] <- dat.df[inds, paste("rural_withdrawal", yyy, sep = ".")] * 0.92
    }
    inds <- which(unlist(dat.df[, paste("mf_return", yyy, sep = ".")]) > 0.92 * unlist(dat.df[, paste("mf_withdrawal", yyy, sep = ".")]))
    if (length(inds) > 0) {
      dat.df[inds, paste("mf_return", yyy, sep = ".")] <- dat.df[inds, paste("mf_withdrawal", yyy, sep = ".")] * 0.92
    }
  }
} else { # baseline
  ...
  dat.df <- cbind(dat.df, do.call(cbind, lapply(yrs, function(yy) {
    cnt <- unique(as.character(dat.df$country))[which(unique(as.character(dat.df$country)) %in% unique(as.character(mf_withdrawal.df$Country_Code)))]
    mfpc <- unlist(sapply(cnt, function(cc) {
      return(mf_withdrawal.df[which(as.character(mf_withdrawal.df$Scenario) == "SSP2" & as.character(mf_withdrawal.df$Country_Code) == cc), paste("X", yy, sep = "")] / sum(dat.df[which(dat.df$country == cc), paste("urban_pop", yy, sep = ".")], na.rm = TRUE))
    }))
    tmp.df <- data.frame(country = dat.df$country)
    tmp.df$res <- NA
    for (x in cnt) {
  ...
  dat.df <- cbind(dat.df, do.call(cbind, lapply(yrs, function(yy) {
    cnt <- unique(as.character(dat.df$country))[which(unique(as.character(dat.df$country)) %in% unique(as.character(mf_return.df$Country_Code)))]
    mfpc <- unlist(sapply(cnt, function(cc) {
      return(mf_return.df[which(as.character(mf_return.df$Scenario) == "SSP2" & as.character(mf_return.df$Country_Code) == cc), paste("X", yy, sep = "")] / sum(dat.df[which(dat.df$country == cc), paste("urban_pop", yy, sep = ".")], na.rm = TRUE))
    }))
    tmp.df <- data.frame(country = dat.df$country)
    tmp.df$res <- NA
    for (x in cnt) {
...
for (reg in c("R11", "R12")) {
  ...
  regional_urban_withdrawal.df <- data.frame(do.call(cbind, lapply(unique(dat.df$region), function(rr) {
    sapply(ysr_full, function(yy) {
      sum(unlist(dat.df[which(dat.df$region == rr), c(paste("urban_withdrawal", yy, sep = "."))]), na.rm = TRUE) + sum(unlist(dat.df[which(dat.df$region == rr), c(paste("mf_withdrawal", yy, sep = "."))]), na.rm = TRUE)
    })
  })))
  ...
  # urban return flows
  regional_urban_return.df <- data.frame(do.call(cbind, lapply(unique(dat.df$region), function(rr) {
    sapply(ysr_full, function(yy) {
      sum(unlist(dat.df[which(dat.df$region == rr), c(paste("urban_return", yy, sep = "."))]), na.rm = TRUE) + sum(unlist(dat.df[which(dat.df$region == rr), c(paste("mf_return", yy, sep = "."))]), na.rm = TRUE)
    })
  })))
  ...
  regional_urban_withdrawal2.df <- data.frame(do.call(cbind, lapply(unique(dat.df$region), function(rr) {
    sapply(ysr_full, function(yy) {
      sum(unlist(dat.df[which(dat.df$region == rr), c(paste("urban_withdrawal", yy, sep = "."))]), na.rm = TRUE)
    })
  })))
  ...
  regional_urban_return2.df <- data.frame(do.call(cbind, lapply(unique(dat.df$region), function(rr) {
    sapply(ysr_full, function(yy) {
      sum(unlist(dat.df[which(dat.df$region == rr), c(paste("urban_return", yy, sep = "."))]), na.rm = TRUE)
    })
  })))
  ...
  regional_manufacturing_withdrawal.df <- data.frame(do.call(cbind, lapply(unique(dat.df$region), function(rr) {
    sapply(ysr_full, function(yy) {
      sum(unlist(dat.df[which(dat.df$region == rr), c(paste("mf_withdrawal", yy, sep = "."))]), na.rm = TRUE)
    })
  })))
  ...
  regional_manufacturing_return.df <- data.frame(do.call(cbind, lapply(unique(dat.df$region), function(rr) {
    sapply(ysr_full, function(yy) {
      sum(unlist(dat.df[which(dat.df$region == rr), c(paste("mf_return", yy, sep = "."))]), na.rm = TRUE)
    })
  })))
  ...
  regional_urban_connection_rate.df <- data.frame(do.call(cbind, lapply(unique(dat.df$region), function(rr) {
    sapply(ysr_full, function(yy) {
      sum(c(unlist(dat.df[which(dat.df$region == rr), c(paste("urban_pop", yy, sep = "."))]) * unlist(dat.df[which(dat.df$region == rr), c(paste("urban_connection_rate", yy, sep = "."))])), na.rm = TRUE) / sum(c(unlist(dat.df[which(dat.df$region == rr), c(paste("urban_pop", yy, sep = "."))])), na.rm = TRUE)
    })
  })))
  ...
  regional_urban_treatment_rate.df <- data.frame(do.call(cbind, lapply(unique(dat.df$region), function(rr) {
    sapply(ysr_full, function(yy) {
      sum(c(unlist(dat.df[which(dat.df$region == rr), c(paste("urban_pop", yy, sep = "."))]) * unlist(dat.df[which(dat.df$region == rr), c(paste("urban_treatment_rate", yy, sep = "."))])), na.rm = TRUE) / sum(c(unlist(dat.df[which(dat.df$region == rr), c(paste("urban_pop", yy, sep = "."))])), na.rm = TRUE)
    })
  })))
  ...
  regional_rural_connection_rate.df <- data.frame(do.call(cbind, lapply(unique(dat.df$region), function(rr) {
    sapply(ysr_full, function(yy) {
      sum(c(unlist(dat.df[which(dat.df$region == rr), c(paste("rural_pop", yy, sep = "."))]) * unlist(dat.df[which(dat.df$region == rr), c(paste("rural_connection_rate", yy, sep = "."))])), na.rm = TRUE) / sum(c(unlist(dat.df[which(dat.df$region == rr), c(paste("rural_pop", yy, sep = "."))])), na.rm = TRUE)
    })
  })))
  ...
  regional_rural_treatment_rate.df <- data.frame(do.call(cbind, lapply(unique(dat.df$region), function(rr) {
    sapply(ysr_full, function(yy) {
      sum(c(unlist(dat.df[which(dat.df$region == rr), c(paste("rural_pop", yy, sep = "."))]) * unlist(dat.df[which(dat.df$region == rr), c(paste("rural_treatment_rate", yy, sep = "."))])), na.rm = TRUE) / sum(c(unlist(dat.df[which(dat.df$region == rr), c(paste("rural_pop", yy, sep = "."))])), na.rm = TRUE)
    })
  })))
  ...
  # rural withdrawals
  regional_rural_withdrawal.df <- data.frame(do.call(cbind, lapply(unique(dat.df$region), function(rr) {
    sapply(ysr_full, function(yy) {
      sum(unlist(dat.df[which(dat.df$region == rr), c(paste("rural_withdrawal", yy, sep = "."))]), na.rm = TRUE)
    })
  })))
  ...
  regional_rural_return.df <- data.frame(
    do.call(cbind, lapply(unique(dat.df$region), function(rr) {
      sapply(ysr_full, function(yy) {
        sum(unlist(dat.df[
          which(dat.df$region == rr),
          c(paste("rural_return", yy, sep = "."))
        ]), na.rm = TRUE)
      })
    }))
  )
  ...
  for (v in names(var_list)) {
    ...
    for_lm <- df %>%
      ...
      group_by(region) %>%
      mutate(
        int = lm(value ~ year)$coefficients[1],
    ...
    write.csv(final_df,
      paste0(
        data_path, "/water_demands/harmonized/", reg, "/ssp", ssp, "_",
        v, "_", ch, ".csv"
      ),
      row.names = TRUE
    )
