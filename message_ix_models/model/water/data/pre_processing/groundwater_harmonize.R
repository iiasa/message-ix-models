...
ggplot() +
  geom_polygon(
    data = table_depth.tidy2, aes(x = long, y = lat, group = group, fill = table_depth_m),
    color = "black"
  )
...
table_depth.out <- table_depth.spdf@data %>%
  mutate(
    table_depth_m = if_else(is.na(table_depth_m),
...
names(gwabstract.stack) <- c(sapply(1:(nlayers(gwabstract.stack) / 12), function(yy) {
  return(paste((as.numeric(unlist(strsplit(as.character(as.Date("1901-01-01") + min(ncvar_get(nc, "time"))), "-"))[1]) + yy - 1), seq(1, 12, by = 1), sep = "."))
}))
...
names(irrigation.stack) <- c(sapply(1:(nlayers(irrigation.stack) / 12), function(yy) {
  return(paste((as.numeric(unlist(strsplit(as.character(as.Date("1901-01-01") + min(ncvar_get(nc, "time"))), "-"))[1]) + yy - 1), seq(1, 12, by = 1), sep = "."))
}))
...
names(industrial.stack) <- c(sapply(1:(nlayers(industrial.stack) / 12), function(yy) {
  return(paste((as.numeric(unlist(strsplit(as.character(as.Date("1901-01-01") + min(ncvar_get(nc, "time"))), "-"))[1]) + yy - 1), seq(1, 12, by = 1), sep = "."))
}))
...
names(domestic.stack) <- c(sapply(1:(nlayers(domestic.stack) / 12), function(yy) {
  return(paste((as.numeric(unlist(strsplit(as.character(as.Date("1901-01-01") + min(ncvar_get(nc, "time"))), "-"))[1]) + yy - 1), seq(1, 12, by = 1), sep = "."))
}))
...
gw_fraction.tidy2 <- dplyr::left_join(gw_fraction.tidy %>% mutate(id = as.integer(id)),
  groundwater_fraction.spdf@data,
  by = "id"
)
...
ggplot() +
  geom_polygon(
    data = gw_fraction.tidy2, aes(x = long, y = lat, group = group, fill = gw_fraction),
    color = "black"
  )
...
irr_dem <- read.csv(paste0("P:/ene.model/NEST/groundwater/hist_irrigation_withdrawals_", reg, ".csv"),
  ...
  gather(key = "year", value = "irr_dem", -node) %>%
  rename(REGION = node) %>%
  mutate(year = as.numeric(year))
# urban + manufacturing and rural demand depend on the region, in MCM!!
urban_man_dem <- read.csv(
  paste0(
    "P:/ene.model/NEST/water_demands/harmonized/",
...
tot_demand <- urban_man_dem %>%
  ...
  group_by(REGION) %>%
  mutate(
    tot_urb_reg_wtr = sum(urb_dem),
    region_water_share = urb_dem / tot_urb_reg_wtr
  ) %>%
  ungroup() %>%
  # use this share to allocate irrigation withdrawals to basins
  mutate(
    irr_dem_bas = irr_dem * region_water_share,
    tot_dem = irr_dem_bas + urb_dem + rur_dem
  )

hist_cap_gw_sw <- tot_demand %>%
  select(BCU_name, tot_dem) %>%
  left_join(groundwater_fraction.spdf@data %>% select(BCU_name, gw_fraction)) %>%
  mutate(
    hist_cap_gw_km3_year = round(tot_dem * gw_fraction, 3),
