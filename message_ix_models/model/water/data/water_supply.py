...
# Helper function to process rules using run_standard
def _process_rules(
    rules: list[dict[str, Any]],
    base_args: dict[str, Any],
    **kwargs: Any,
) -> list[pd.DataFrame]:
    """Iterate through rules, run standard processing, and collect results."""
    outputs = []
    for r in rules:
        # Prepare arguments for run_standard
        current_args = base_args.copy()
        current_kwargs = kwargs.copy()

        # Update base_args with rule-specific data if needed (example shown)
        # if 'some_key' in r:
        #     current_args['rule_dfs'] = r['some_key']
        # # Adjust as per actual rule structure

        # Handle specific arguments passed via kwargs
        if "extra_args_func" in current_kwargs:
            extra_args = current_kwargs.pop("extra_args_func")(r)
            current_kwargs["extra_args"] = extra_args
        if "broadcast_year_func" in current_kwargs:
            broadcast_year = current_kwargs.pop("broadcast_year_func")(r)
            current_kwargs["broadcast_year"] = broadcast_year
        if "sub_time_func" in current_kwargs:
            sub_time = current_kwargs.pop("sub_time_func")(r)
            current_args["sub_time"] = sub_time

        df_rule = run_standard(r, current_args, **current_kwargs)
        outputs.append(df_rule)
    return outputs
