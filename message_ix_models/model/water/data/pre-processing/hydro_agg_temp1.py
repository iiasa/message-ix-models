...
wd1 = r"C:\Users\awais\Documents\GitHub\agg_data"
wd = r"P:\watxene\ISIMIP\ISIMIP2b\output\LPJmL"
wd2 = r"P:\ene.model\NEST\hydrology\post-processed"
...
if multimodelensemble:
    ...
    hydro_data = wd + f"\*{climmodel}*{scen}*{var}*.nc4"
    files = glob.glob(hydro_data)
    if var != "qr":
        ...
        # Open hydrological data as a combined dataset
        da = xr.open_mfdataset(files, decode_times=False)
        da["time"] = pd.date_range(start="1/1/2006", end="31/12/2099", freq="M")
...
if monthlyscale:
    ...
    if var == "dis":
        ...
        da.to_netcdf(wd2 + f"{var}_monthly__{climmodel}_{scen}.nc")
    elif var == "qtot":
        ...
        da.to_netcdf(wd2 + f"\output\{var}_monthly_{climmodel}_{scen}.nc")
    elif var == "qr":
        # Filter 5 year interval monthly time step
        # da = da.sel(time=da.time.dt.year.isin(years))
        # 1kg/m2/sec = 86400 mm/day
        ...
        da.to_netcdf(wd2 + f"\output\{var}_5y_m_{climmodel}_{scen}.nc")
