...
for scen in scenarios:
    ...
    df = pd.read_csv(
        wd + f"\{var}_monthly_{climmodels[0]}_{scen}.csv").iloc[:,8:].set_axis(pd.date_range(
            '2006-1-1', '2099-12-31', freq = 'M'), axis=1)
    ...
    df2 = pd.read_csv(
        wd + f"\{var}_monthly_{climmodels[1]}_{scen}.csv").iloc[:,8:].set_axis(pd.date_range(
            '2006-1-1', '2099-12-31', freq = 'M'), axis=1)

    df3 = pd.read_csv(
        wd + f"\{var}_monthly_{climmodels[2]}_{scen}.csv").iloc[:,8:].set_axis(pd.date_range(
            '2006-1-1', '2099-12-31', freq = 'M'), axis=1)
    ...
    if GCM == 'dry':
        ...
        dfs_dry[scen].to_csv(wd + f"\{var}_5ymonthly_{GCM}_{extreme_scen}_{scen}_{intp}.csv")
        ...
        #         level=0).mean()
        dffinal[scen].to_csv(wd + f"\{var}_5ymonthly_{GCM}_{extreme_scen}_{scen}_{intp}.csv")
        print("5 year Monthly values and quantiles processed")
        # Store for next processing
        df = dffinal[scen]
        # Convert to 5 year annual values 
