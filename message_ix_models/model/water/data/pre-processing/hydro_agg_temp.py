...
# 
qtot_7p0_gfdl = pd.read_csv(wd11 + f"\{var}_monthly_gfdl-esm4_ssp370_future.csv").drop(["Unnamed: 0", 
                                                                                  "NAME", "BASIN", "BASIN_ID",
                                                                                  "area", "area_km2","X","REGION","BCU_name"], axis = 1)

new_cols = pd.date_range("2015-01-01", periods=1032, freq="M")  
qtot_7p0_gfdl.columns = new_cols


qtot_2p6_gfdl = pd.read_csv(wd11 + f"\{var}_monthly_gfdl-esm4_ssp126_future.csv").drop(["Unnamed: 0", 
                                                                                  "NAME", "BASIN", "BASIN_ID",
                                                                                  "area", "area_km2","X","REGION","BCU_name"], axis = 1)
...
val_2020 = (qtot_7p0_gfdl_avg.groupby(qtot_7p0_gfdl_avg.columns.month, axis = 1).mean() +
               qtot_2p6_gfdl_avg.groupby(qtot_2p6_gfdl_avg.columns.month, axis = 1).mean())/2
val_2020_annual = val_2020.mean(axis =1)

delta60 = qtot_7p0_gfdl_avg.groupby(qtot_7p0_gfdl_avg.columns.month, axis = 1).mean() - val_2020
delta26 = qtot_2p6_gfdl_avg.groupby(qtot_2p6_gfdl_avg.columns.month, axis = 1).mean() - val_2020
        ...
        if year == 2020:
            ...
            if delta_multiply > 0.1:
                ...
                for i in np.arange(4, -1, -1):
                    ...
                    if i == 4:
                        delta60.columns = pd.date_range(str(year - i) + "-01-01", periods = 12, freq = "M")
                        final_temp = df[pd.date_range(str(year - i) + "-01-01", periods = 12, freq = "M")] \
                        - (delta_multiply * delta60)
                    
                    else:
                        delta60.columns = pd.date_range(str(year - i) + "-01-01", periods = 12, freq = "M")
                        temp = df[pd.date_range(str(year - i) + "-01-01", periods = 12, freq = "M")] \
                        - (delta_multiply * delta60)

                        final_temp = pd.concat((final_temp, temp), axis = 1)
                
                df_monthly = final_temp
                df[pd.date_range(str(year) + "-01-01", periods = 12, freq = "M")] = final_temp.groupby(
                    final_temp.columns.month, axis = 1).mean()
                # 5 year monthly data
                df_5y_m = df[df.columns[df.columns.year.isin(years)]]
                # 5 year annual
                # 50th quantile - q50 
                df_q50 = df_monthly.rolling(240,
                        min_periods = 1,
                        axis = 1).quantile(
                            0.5, interpolation= 'linear').resample('5Y', axis = 1).mean() 
                df_q50['2020-12-31'] = val_2020_annual
                # 70th quantile - q70
                df_q70 = df_monthly.rolling(240,
                        min_periods = 1,
                        axis = 1).quantile(
                            0.3, interpolation= 'linear').resample('5Y', axis = 1).mean()  
                df_q70['2020-12-31'] = val_2020_annual
                # 90th quantile - q90
                df_q90 = df_monthly.rolling(240,
                        min_periods = 1,
                        axis = 1).quantile(
                            0.1, interpolation= 'linear').resample('5Y', axis = 1).mean() 
                ...
                # after detlta years
                temp_daterange = pd.date_range(str(year-4) + "-01-01", periods = 60, freq = "M")
                # Monthly Bias corrected data   
                df_monthly = df

                df[pd.date_range(str(year) + "-01-01", periods = 12, freq = "M")] = df[temp_daterange].groupby(
                    df[temp_daterange].columns.month, axis = 1).mean()
                # 5 year monthly
                df_5y_m = df[df.columns[df.columns.year.isin(years)]]
                # 5 year annual
                # 50th quantile - q50 
...
df_26, df_monthly_26, df_5y_m_26 , df_q50_26, df_q70_26, df_q90_26   = bias_correction(qtot_2p6_gfdl)

df_70, df_monthly_70, df_5y_m_70 , df_q50_70, df_q70_70, df_q90_70   = bias_correction(qtot_7p0_gfdl)
...
df_q50_70.apply(lambda x:val_2020_annual).to_csv(wd11 + f'/{var}_5y_no_climate_low_{iso3}.csv')
df_q70_70.apply(lambda x:val_2020_annual).to_csv(wd11 + f'/{var}_5y_no_climate_med_{iso3}.csv')
df_q90_70.apply(lambda x:val_2020_annual).to_csv(wd11 + f'/{var}_5y_no_climate_high_{iso3}.csv')
...
for year in np.arange(2020, 2105, 5):
    for i in np.arange(4, -1, -1):
        ...
        if i == 4:
            ...
            final_temp = eflow[pd.date_range(str(year - i) + "-01-01", periods = 12, freq = "M")]
    ...
    df_monthly = final_temp
    eflow[pd.date_range(str(year) + "-01-01", periods = 12, freq = "M")] = final_temp.groupby(
        final_temp.columns.month, axis = 1).mean()
