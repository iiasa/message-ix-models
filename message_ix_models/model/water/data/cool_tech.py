...
async def cool_tech(context: "Context") -> dict[str, pd.DataFrame]:
    ...
    df_node = await asyncio.to_thread(pd.read_csv, PATH)
    ...
    ref_input = await asyncio.to_thread(
        scen.par, "input", {"technology": cooling_df["parent_tech"]}
    )
    ...
    ref_output = await asyncio.to_thread(
        scen.par, "output", {"technology": missing_tec}
    )
    ...
    df_sw = await asyncio.to_thread(map_basin_region_wat, context)
    ...
    cost = await asyncio.to_thread(pd.read_csv, path1)
    ...
    cap_fact_parent = await asyncio.to_thread(
        scen.par, "capacity_factor", {"technology": cooling_df["parent_tech"]}
    )
    ...
    cost_proj = await asyncio.to_thread(create_cost_projections, cfg)
    ...
    # ]
    # Define tasks for fetching parameters asynchronously
    tasks = [
        asyncio.to_thread(scen.par, p, {"technology": cooling_df["parent_tech"]})
        for p in param_names
    ]
    # Run tasks concurrently and get results
    param_data_list = await asyncio.gather(*tasks)
    ...
    df_share = await asyncio.to_thread(cooling_shares_SSP_from_yaml, context)
