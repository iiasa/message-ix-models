...
def _prepare_initial_cooling_data(
    ...
    electr.loc[:, "value_cool"] = (
        electr["parasitic_electricity_demand_fraction"]
        / electr["cooling_fraction"]
    )
    # Set to 1e-6 if value_cool is negative
...
def cool_tech(context: "Context") -> dict[str, pd.DataFrame]:
    ...
    out =  _generate_output_parameter(
        context, input_cool, icmse_df, df_node, sub_time
    )
    results["output"] = out
...
def non_cooling_tec(context: "Context") -> dict[str, pd.DataFrame]:
    ...
    non_cool_df["value"] = (
        ...
        * COOL_CONST["SECINYEAR"]* COOL_CONST["M3toMCM"]
    )
