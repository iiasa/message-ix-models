...
def import_inp(node_path, version, verbose):
    ...
    log.info(f'Importing from {len(node_path)} nodes × {len(tecs)} technology/'
             'consumer groups')
...
def load_inp(version):
    ...
    return pd.read_csv(data_path / f'{version}.csv.gz',
...
def fill_year_act(data, info):
    ...
    data = data.set_index('year_act', append=True) \
               .unstack('year_act') \
...
def truncate(data, info):
    ...
    col = next(filter(lambda c: c in data.index.names,
                      ['year_vtg', 'year']))

    log.info('Years ' + str(data.index.to_frame()[col].unique()) +
             f' < {info.y0}')
