...
inv_cost = te_params[2010][
    list(range(0, te_params.shape[0], n_inputs_per_tech))
].reset_index(drop=True)
fix_cost = te_params[2010][
    list(range(1, te_params.shape[0], n_inputs_per_tech))
].reset_index(drop=True)
var_cost = te_params[2010][
    list(range(2, te_params.shape[0], n_inputs_per_tech))
].reset_index(drop=True)
technical_lifetime = te_params[2010][
    list(range(3, te_params.shape[0], n_inputs_per_tech))
].reset_index(drop=True)
...
input_elec = te_params[2010][
    list(range(5, te_params.shape[0], n_inputs_per_tech))
].reset_index(drop=True)
input_elec = input_elec * 0.0317  # 0.0317 GWa/PJ
input_water = te_params[2010][
    list(range(6, te_params.shape[0], n_inputs_per_tech))
].reset_index(drop=True)
output_NH3 = te_params[2010][
    list(range(7, te_params.shape[0], n_inputs_per_tech))
].reset_index(drop=True)
output_water = te_params[2010][
    list(range(8, te_params.shape[0], n_inputs_per_tech))
].reset_index(drop=True)
output_heat = te_params[2010][
    list(range(9, te_params.shape[0], n_inputs_per_tech))
].reset_index(drop=True)
output_heat = output_heat * 0.0317  # 0.0317 GWa/PJ
emissions = (
    te_params[2010][list(range(10, te_params.shape[0], n_inputs_per_tech))].reset_index(
        drop=True
    )
    * 12
    / 44
...
N_demand_GLO = pd.read_excel(
    r"..\CD-Links SSP2 N-fertilizer demand.Global.xlsx", sheet_name="data"
)
...
feedshare_GLO = pd.read_excel(
    r"..\Ammonia feedstock share.Global.xlsx", sheet_name="Sheet2", skiprows=13
)
...
N_trade_R11 = N_trade_R11.rename(
    columns={
        "Value": "value",
        "Unit": "unit",
        "msgregion": "node_loc",
