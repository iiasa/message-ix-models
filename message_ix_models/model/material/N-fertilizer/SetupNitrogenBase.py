...
params_src = [
    x for x in paramList_tec if "solar_i" in set(Sc_ref.par(x)["technology"].tolist())
]
...
for t in newtechnames[0:5]:
    ...
    df = Sc_nitro.par(
        "technical_lifetime", {"technology": ["solar_i"]}
    )  # lifetime = 15 year
    df["technology"] = t
    ...
    df = Sc_nitro.par(
        "relation_activity", {"relation": ["CO2_cc"], "technology": ["h2_smr"]}
    )
    df["value"] = LoadParams.emissions[newtechnames.index(t)]
    df["technology"] = t
    df["unit"] = "???"
...
N_demand = (
    N_demand_raw[N_demand_raw.Scenario == "NoPolicy"].reset_index().loc[0:10, 2010]
)  # 2010 tot N demand
...
df = Sc_nitro.par("historical_activity").iloc[
    0 : len(newtechnames) * (len(REGIONS) - 1),
]  # Choose whatever same number of rows
...
df = Sc_nitro.par("historical_new_capacity").iloc[
    0 : len(newtechnames) * (len(REGIONS) - 1),
]  # whatever
df["technology"] = newtechnames * (len(REGIONS) - 1)
df["value"] = [
    x * 1 / life / LoadParams.capacity_factor[0] for x in act2010
]  # Assume 1/lifetime (=15yr) is built each year
df["year_vtg"] = 2010
df["node_loc"] = [y for x in REGIONS[:-1] for y in (x,) * len(newtechnames)]
df["unit"] = "Tg N/yr"
...
years = set(map(int, list(Sc_nitro.set("year")))) & set(
    N_demand_raw
)  # get intersection
...
df = demand_fs_org.loc[demand_fs_org.year == 2010, :].join(
    LoadParams.N_energy.set_index("node"), on="node"
)
sh = pd.DataFrame(
    {
...
newtechnames_ccs = list(
    map(lambda x: str(x) + "_ccs", tec_for_ccs)
)  # include biomass in CCS, newtechnames[2:5]))
...
df.loc[df["commodity"] == "electr", ["value"]] = (
    df.loc[df["commodity"] == "electr", ["value"]] + 0.005
)  # TUNE THIS # Add electricity input for CCS
Sc_nitro.add_par("input", df)
...
h2_smr_vent_ratio = (
    -Sc_nitro.par(
        "relation_activity", {"relation": ["CO2_cc"], "technology": ["h2_smr_ccs"]}
    ).value[0]
    / Sc_nitro.par(
        "relation_activity",
        {"relation": ["CO2_Emission"], "technology": ["h2_smr_ccs"]},
    ).value[0]
)
h2_coal_vent_ratio = (
    -Sc_nitro.par(
        "relation_activity", {"relation": ["CO2_cc"], "technology": ["h2_coal_ccs"]}
    ).value[0]
    / Sc_nitro.par(
        "relation_activity",
        {"relation": ["CO2_Emission"], "technology": ["h2_coal_ccs"]},
    ).value[0]
)
h2_bio_vent_ratio = 1 + Sc_nitro.par(
    "relation_activity", {"relation": ["CO2_cc"], "technology": ["h2_bio_ccs"]}
).value[0] / (
    Sc_nitro.par(
        "input", {"technology": ["h2_bio_ccs"], "commodity": ["biomass"]}
    ).value[0]
    * biomass_ef
)
...
df_coal.loc[(df.emission == "CO2_transformation"), "value"] = (
    ef_org * h2_coal_vent_ratio
)
...
biomass_input = Sc_nitro.par(
    "input", {"technology": ["biomass_NH3"], "commodity": ["biomass"]}
).value[0]
...
a = df[df["technology"].str.contains("h2_smr")].sort_values(
    ["technology", "year_vtg"]
)  # To get cost ratio for std vs CCS
r_ccs_cost_gas = (
    a.loc[(a.technology == "h2_smr") & (a.year_vtg >= 2020)]["value"].values
    / a.loc[(a["technology"] == "h2_smr_ccs") & (a.year_vtg >= 2020)]["value"].values
)
r_ccs_cost_gas = r_ccs_cost_gas.mean()

a = df[df["technology"].str.contains("h2_coal")].sort_values(
    ["technology", "year_vtg"]
)  # To get cost ratio for std vs CCS
r_ccs_cost_coal = (
    a.loc[(a.technology == "h2_coal") & (a.year_vtg >= 2020)]["value"].values
    / a.loc[(a["technology"] == "h2_coal_ccs") & (a.year_vtg >= 2020)]["value"].values
)
r_ccs_cost_coal = r_ccs_cost_coal.mean()

a = df[df["technology"].str.contains("h2_bio")].sort_values(
    ["technology", "year_vtg"]
)  # To get cost ratio for std vs CCS
a = a[a.year_vtg > 2025]  # h2_bio_ccs only available from 2030
r_ccs_cost_bio = (
    a.loc[(a.technology == "h2_bio") & (a.year_vtg >= 2020)]["value"].values
    / a.loc[(a["technology"] == "h2_bio_ccs") & (a.year_vtg >= 2020)]["value"].values
)
...
df_gas["technology"] = df_gas["technology"] + "_ccs"  # Rename the technologies
df_gas["value"] = (
    df_gas["value"] / r_ccs_cost_gas
)  # Same scaling (same 4% of inv_cost in the end)
Sc_nitro.add_par("fix_cost", df_gas)

df_coal = df[df["technology"].isin(tec_for_ccs[2:])]
df_coal["technology"] = df_coal["technology"] + "_ccs"  # Rename the technologies
df_coal["value"] = (
    df_coal["value"] / r_ccs_cost_coal
)  # Same scaling (same 4% of inv_cost in the end)
...
df_gas = df[
    df["technology"] == "gas_NH3"
]  # Originally all CO2_cc (truly emitted, bottom-up)
...
# Coal / Oil
df_coal = df[
    df["technology"].isin(tec_for_ccs[2:])
]  # Originally all CO2_cc (truly emitted, bottom-up)
...
# Biomass
df_bio = df[
    df["technology"] == "biomass_NH3"
]  # Originally all CO2.cc (truly emitted, bottom-up)
df_bio.value = biomass_input * (h2_bio_vent_ratio - 1) * biomass_ef
df_bio = df_bio.assign(technology=df_bio.technology + "_ccs")
...
scaler_cost = pd.DataFrame(
    {
        ...
        "scaler_ccs": [
            ...
            0.70,
            1.05,
        ],  # NA values are replaced with WEO 2014 values. (LAM & MEA = 0.80)
        "node_loc": [
            "R11_NAM",
            "R11_LAM",
            "R11_WEU",
...
scalers = [
    [0.66 * 0.91, 1, 0.75 * 0.9],
    [0.59, 1, 1],
]  # f, g, c : Scale based on the original techno-economic assumptions
...
for p in ["inv_cost", "fix_cost"]:  # , 'var_cost']:
    for r in reg2scale:
        ...
        df_fc.value *= scalers[reg2scale.index(r)][
            0
        ]  # Gas/fueloil cost the same as coal
        df_gc.value *= scalers[reg2scale.index(r)][1]
        df_cc.value *= scalers[reg2scale.index(r)][2]

        df_f.value *= scalers[reg2scale.index(r)][
            0
        ]  # Gas/fueloil cost the same as coal
...
Sc_nitro.solve(
    model="MESSAGE",
    case=Sc_nitro.model + "_" + Sc_nitro.scenario + "_" + str(Sc_nitro.version),
)
