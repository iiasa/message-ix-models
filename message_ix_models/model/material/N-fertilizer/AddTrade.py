...
comment = (
    "MESSAGE global test for new representation of nitrogen cycle with global trade"
)
...
Sc_nitro_trd.add_set(
    "technology", newtechnames_trd + newtechnames_imp + newtechnames_exp
)
...
for t in newtechnames_imp:
    # output
    df = Sc_nitro_trd.par(
        "output", {"technology": ["coal_imp"], "node_loc": ["R11_CPA"]}
    )
    df["technology"] = t
    ...
    # input
    df = Sc_nitro_trd.par(
        "input", {"technology": ["coal_imp"], "node_loc": ["R11_CPA"]}
    )
    df["technology"] = t
    df["commodity"] = comm_for_trd[newtechnames_imp.index(t)]
    df["value"] = 1
...
for t in newtechnames_exp:
    # output
    df = Sc_nitro_trd.par(
        "output", {"technology": ["coal_exp"], "node_loc": ["R11_CPA"]}
    )
    df["technology"] = t
    ...
    # input
    df = Sc_nitro_trd.par(
        "input", {"technology": ["coal_exp"], "node_loc": ["R11_CPA"]}
    )
    df["technology"] = t
    df["commodity"] = comm_for_trd[newtechnames_exp.index(t)]
    df["value"] = 1
...
paramList_tec = [
    x for x in Sc_nitro_trd.par_list() if "technology" in Sc_nitro_trd.idx_sets(x)
]
...
a = a - set(
    [x for x in a if x.endswith(suffix)]
    + [x for x in a if x.startswith(prefix)]
    + ["emission_factor"]
)
...
for t in newtechnames_exp:
    df = Sc_nitro_trd.par(
        "technical_lifetime", {"technology": t, "node_loc": ["R11_CPA"]}
    )
    for r in reg:
        df["node_loc"] = r
...
df_histexp = N_trade.loc[
    (N_trade.Element == "Export") & (N_trade.year_act < 2015),
]
...
df_histimp = N_trade.loc[
    (N_trade.Element == "Import") & (N_trade.year_act < 2015),
]
...
dftrd = dftrd.loc[
    (dftrd.year_act < 2015) & (dftrd.year_act > 2000),
]
...
df = demand_fs_org.loc[demand_fs_org.year == 2010, :].join(
    LoadParams.N_feed.set_index("node"), on="node"
)
sh = pd.DataFrame(
    {
...
Sc_nitro_trd.commit(
    "Nitrogen Fertilizer for global model with fertilizer trade via global pool"
)
...
Sc_nitro_trd.solve(
    model="MESSAGE",
    case=Sc_nitro_trd.model
    + "_"
    + Sc_nitro_trd.scenario
    + "_"
    + str(Sc_nitro_trd.version),
)
