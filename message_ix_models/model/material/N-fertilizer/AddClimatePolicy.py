...
comment = (
    "MESSAGE_Global test for new representation of nitrogen cycle with climate policy"
)
...
df_ACT = (
    Sc_nitro_2C.var("ACT", {"year_act": lasthistyear})
    .groupby(["node_loc", "technology", "year_act"])
    .sum()
    .reset_index()
)
df_CAP_NEW = (
    Sc_nitro_2C.var("CAP_NEW", {"year_vtg": lasthistyear})
    .groupby(["node_loc", "technology", "year_vtg"])
    .sum()
    .reset_index()
)
df_EXT = (
    Sc_nitro_2C.var("EXT", {"year": lasthistyear})
    .groupby(["node", "commodity", "grade", "year"])
    .sum()
    .reset_index()
)
...
fyear = 2030  # 2025
a = df[
    ((df.type_year == "cumulative") & (df.year < fyear))
    | ((df.type_year == "firstmodelyear") & (df.year < fyear))
]
...
# df = df[df.value > 0]
a = pd.merge(
    df[["node_loc", "technology", "mode", "time", "unit"]],
    df_ACT[["node_loc", "technology", "year_act", "lvl"]],
    how="left",
    on=["node_loc", "technology"],
...
# df = df[df.value > 0]
a = pd.merge(
    df[["node_loc", "technology", "unit"]],
    df_CAP_NEW[["node_loc", "technology", "year_vtg", "lvl"]],
    how="left",
    on=["node_loc", "technology"],
).rename(columns={"lvl": "value"})
a = a[a.value > 0]
...
# df = df[df.value > 0]
a = pd.merge(
    df[["node", "commodity", "grade", "unit"]],
    df_EXT[["node", "commodity", "grade", "year", "lvl"]],
    how="outer",
    on=["node", "commodity", "grade"],
).rename(columns={"lvl": "value"})
a = a[a.value > 0]
a["unit"] = "GWa"
...
rep.set_filters(
    t=newtechnames_ccs + newtechnames + ["NFert_imp", "NFert_exp", "NFert_trd"]
)
