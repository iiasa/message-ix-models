...
ix.reporting.configure(
    units={"replace": {"???": "", "-": ""}}
)  # '???' and '-' are not handled in pyint(?)
...
def GenerateOutput(model, scen, rep):
    ...
    rep.set_filters()
    rep.set_filters(
        t=(newtechnames_ccs + newtechnames)[:-1], e=["CO2_transformation"]
    )  # and NH3_to_N_fertil does not have emission factor
    ...
    rep.set_filters()
    rep.set_filters(
        t=(newtechnames_ccs + newtechnames),
        c=["coal", "gas", "electr", "biomass", "fueloil"],
    )
...
def plot_NF_result(case="nf_demand", model=model_name, scen="baseline"):
    ...
    if case in ["nf_demand", "all"]:
        data = pyam.IamDataFrame(
            data="nf_demand" + "_" + model + "_" + scen + ".xlsx", encoding="utf-8"
        )
    ...
    if case in ["nf_input", "all"]:
        data = pyam.IamDataFrame(
            data="nf_input" + "_" + model + "_" + scen + ".xlsx", encoding="utf-8"
        )
    ...
    if case in ["nf_emissions_CO2", "all"]:
        data = pyam.IamDataFrame(
            data="nf_emissions_CO2" + "_" + model + "_" + scen + ".xlsx",
            encoding="utf-8",
        )
        for v in list(data.variables()):
            data.aggregate_region(v, append=True)
    ...
    if case in ["price_commodity", "all"]:
        # N fertilizer
        data = pyam.IamDataFrame(
            data="price_commodity" + "_" + model + "_" + scen + ".xlsx",
            encoding="utf-8",
        )
        fig, ax = plt.subplots(figsize=(12, 12))
        data.filter(variable="Price|Fertilizer Use|*", region="R11_AFR").line_plot(
            ax=ax, legend=False
        )  # Identical across regoins through trade
        plt.savefig("./plots/price_NF" + "_" + model + "_" + scen + ".png")

        # NH3
        fig, ax = plt.subplots(figsize=(12, 12))
        data.filter(variable="Price|NH3").line_plot(
            ax=ax, color="region", legend=OUTSIDE_LEGEND["right"]
        )
