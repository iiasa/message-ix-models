...
def report(scenario, print_to_excel=False):  # noqa: C901
    ...
    df.divide(
        "in|final|charcoal|concrete_production_cement|M2",
        (0.951),
        "in|final|charcoal|concrete_production_cement|Mt",
        append=True,
        ignore_units=True,
    )
    ...
    for r in nodes:
        ...
        df_steel.aggregate(
            "Production|Primary|Steel (before sub.1)",
        ...
        df_steel.subtract(
            "Production|Primary|Steel (before sub.1)",
            "in|new_scrap|steel|bof_steel|M1",
            "Production|Primary|Steel (before sub.2)",
            append=True,
        )
    ...
    for r in nodes:
        ...
        landfilling_cement_vars = [
            "in|dummy_end_of_life|concrete|landfilling_cement|M1"
        ]

        recycling_cement_vars = [
            "in|dummy_end_of_life|concrete|recycling_cement|M1",
            "in|dummy_end_of_life|concrete|recycling_cement|M2",
        ]
        ...
        for r in nodes:
            ...
            if c == "solids":
                ...
                filter_vars = [
                    v
                    for v in aux2_df["variable"].values
                    if (("biomass" not in v) & ("charcoal" not in v))
                ]
                df_final_energy.aggregate(
