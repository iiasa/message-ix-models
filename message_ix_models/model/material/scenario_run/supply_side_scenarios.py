...
def industry_sector_net_zero_targets(scenario):
    ...
    remove_growth_activity_up = scenario.par(
        "growth_activity_up",
        filters={
            "technology": ["dri_gas_steel", "dri_h2_steel", "eaf_steel"],
            "year_act": remove_years,
        },
    )
    remove_initial_activity_up = scenario.par(
        "initial_activity_up",
        filters={
            "technology": ["dri_gas_steel", "dri_h2_steel", "eaf_steel"],
            "year_act": remove_years,
    ...
    remove_growth_activity_lo = scenario.par(
        "growth_activity_lo",
        filters={"technology": ["bof_steel"], "year_act": remove_years},
    )
    remove_initial_activity_lo = scenario.par(
        "initial_activity_lo",
        filters={"technology": ["bof_steel"], "year_act": remove_years},
    )
    ...
    growth_new_capacity_up = scenario.par(
    ...
    initial_new_capacity_up = scenario.par(
...
def increased_recycling(scenario):
    ...
    relation_recycling = scenario.par(
        "relation_activity",
        filters={
            "relation": ["maximum_recycling_aluminum", "max_regional_recycling_steel"],
            "technology": ["total_EOL_steel", "total_EOL_aluminum"],
        },
    )
    ...
    recycling_costs_steel = scenario.par(
        "var_cost", filters={"technology": ["prep_secondary_steel_1"]}
    )
    recycling_costs_alu = scenario.par(
        "var_cost", filters={"technology": ["prep_secondary_aluminum_1"]}
    )

    recycling_costs_steel_remove = scenario.par(
        "var_cost",
        filters={"technology": ["prep_secondary_steel_2", "prep_secondary_steel_3"]},
    )
    recycling_costs_alu_remove = scenario.par(
        "var_cost",
        filters={
            "technology": ["prep_secondary_aluminum_2", "prep_secondary_aluminum_3"]
        },
    )
    ...
    relation_recycling_concrete = scenario.par(
        "relation_activity",
        filters={
            "relation": ["max_regional_recycling_cement"],
...
def keep_fuel_share(scenario):
    ...
    df = pd.read_excel(
        package_data_path("material", "infrastructure", "df_ratios_final.xlsx")
    )
            ...
            for f in df_sector["fuel"].unique():
                ...
                if s == "Non-Metallic Minerals":
                    ...
                    value_df = df_sector[
                        (df_sector["Region"] == n)
                        & (df_sector["fuel"] == f)
                        & (df_sector["sector"] == "Non-Metallic Minerals")
                    ]
                    val = value_df["Fuel_Ratio"].values[0]
                ...
                if s == "Non-Ferrous Metals":
                    ...
                    value_df = df_sector[
                        (df_sector["Region"] == n)
                        & (df_sector["fuel"] == f)
                        & (df_sector["sector"] == "Non-Ferrous Metals")
                    ]
                    val = value_df["Fuel_Ratio"].values[0]
...
def minimum_ccs_cement(scenario):
    ...
    df = pd.read_excel(
        package_data_path("material", "infrastructure", "clinker_dry_ccs_cement.xlsx")
    )
    ...
    for n in nodes:
        for y in year_act:
            val = df_summed.loc[
                (df_summed["node_loc"] == n) & (df_summed["year_act"] == y), "value"
            ]
            bound_activity = pd.DataFrame(
                {
                    "node_loc": n,
