...
scenario = message_ix.Scenario(
    mp, model="MESSAGE_material", scenario="baseline", version="new"
)
# Addition of basics
...
scenario.add_horizon(
    {"year": history + model_horizon, "firstmodelyear": model_horizon[0]}
)
country = "CHN"
...
duration_period = pd.DataFrame({"year": model_horizon, "value": val, "unit": "y",})
...
for tec in dummy_tech:
    out = make_df("output", technology=tec, commodity=commodity_tec[t], **base)
    t = t + 1
...
fin_to_useful = scenario.par(
    "output", filters={"technology": "finishing_aluminum", "year_act": 2020}
)["value"][0]
useful_to_product = scenario.par(
    "output", filters={"technology": "manuf_aluminum", "year_act": 2020}
)["value"][0]
i = 0
values = []
    ...
    hist_activity = pd.DataFrame(
        {
            "node_loc": country,
            "year_act": history,
            "mode": data_aluminum_hist.loc[
                (data_aluminum_hist["technology"] == tec), "mode"
            ],
            "time": "year",
            "unit": "Mt",
            "technology": tec,
            "value": data_aluminum_hist.loc[
                (data_aluminum_hist["technology"] == tec), "production"
            ],
        }
    )
...
for tec in data_aluminum_hist["technology"].unique():
    ...
    value = (
        data_aluminum_hist.loc[
            (data_aluminum_hist["technology"] == tec), "new_production"
        ]
        / c_factor
    )
...
high_th = scenario.par(
    "input", filters={"technology": "secondary_aluminum", "year_act": 2020}
)["value"][0]
low_th = scenario.par(
    "input", filters={"technology": "prep_secondary_aluminum", "year_act": 2020}
)["value"][0]

historic_generation = scenario.par("historical_activity").groupby("year_act").sum()

for yr in historic_generation.index:

    total_scrap = (
        historic_generation.loc[yr].value * fin_to_useful * (1 - useful_to_product)
    ) + (
        historic_generation.loc[yr] * fin_to_useful * useful_to_product * scrap_recovery
    )
    old_scrap = (
        historic_generation.loc[yr] * fin_to_useful * useful_to_product * scrap_recovery
    )
    ...
    c_fac_furnace_gas = scenario.par(
        "capacity_factor", filters={"technology": "furnace_gas_aluminum"}
    )["value"].values[0]
...
p.plot_activity(
    baseyear=True,
    subset=["soderberg_aluminum", "prebake_aluminum", "secondary_aluminum"],
)
