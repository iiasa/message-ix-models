...
def gen_data_aluminum(file):
    ...
    data_aluminum_hist = pd.read_excel(
        "aluminum_techno_economic.xlsx", sheet_name="data_historical", usecols="A:F"
    )
    # Clean the data
    ...
    for t in technology_add:
        ...
        if (
            "technical_lifetime"
            in data_aluminum.loc[(data_aluminum["technology"] == t)]["parameter"].values
        ):
            lifetime = data_aluminum.loc[
                (data_aluminum["technology"] == t)
                & (data_aluminum["parameter"] == "technical_lifetime"),
                "value",
            ].values[0]
