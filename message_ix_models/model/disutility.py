...
def get_spec(
    ...
    for t, g in product(technologies, groups):
        ...
        input = {
            k: v.format(**fmt) for k, v in template.eval_annotation("input").items()
        }
        output = {
            k: v.format(**fmt) for k, v in template.eval_annotation("output").items()
        }
...
def data_conversion(info, spec) -> MutableMapping[str, pd.DataFrame]:
    ...
    for t in technology:
        ...
        input = t.eval_annotation("input")
        output = t.eval_annotation("output")
