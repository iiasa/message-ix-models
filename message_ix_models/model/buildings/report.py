# ----------------------------------------
# REPORTING TOOL:
# Adds final energy and emissions
# time series to the scenario and re-solve
# -----------------------------------------
import os
...
mod_new = "MESSAGEix-GLOBIOM 1.1-BM-R12-EFC"
scen_new = "baseline"
version = 24
# scen_new = "test_"+clim_scen


scenario = message_ix.Scenario(mp_ENE, mod_new, scen_new, version)
...
FE_rep.loc[FE_rep["technology"] == "biomass_nc", "technology"] = "biomass_nc_resid_cook"


FE_rep["commodity"] = FE_rep["technology"].str.rsplit("_", 1, expand=True)[0]
FE_rep = FE_rep.rename(columns={"year_act": "year", "node_loc": "node", "lvl": "value"})
...
emiss = scenario.par(
    "relation_activity", filters={"technology": build_ene_tecs, "year_act": years_model}
)
...
# Some fixes
emiss.loc[emiss["technology"] == "biomass_nc", "technology"] = "biomass_nc_resid_cook"
...
emiss_rep = emiss_rep.sort_values(["node", "variable", "year"]).reset_index(drop=True)

# Add STURM reporting

os.chdir(r"C:\Users\guof\Documents\Github_IIASA\MESSAGE_Buildings\STURM_output")

if "baseline" in scen_new:
    sturm_res = pd.read_csv("report_NGFS_SSP2_BL_resid_R12.csv")
    sturm_com = pd.read_csv("report_NGFS_SSP2_BL_comm_R12.csv")
else:
    sturm_res = pd.read_csv("../STURM_output/report_IRP_SSP2_2C_resid_R12.csv")
    sturm_com = pd.read_csv("../STURM_output/report_IRP_SSP2_2C_comm_R12.csv")
...
var_list = [
    "Energy Service|Residential and Commercial|Commercial",
...
test_2 = test_2.drop(columns=["sector", "variable_rest", "variable_head", "variable"])
...
test_2["variable"] = test_2["new_var_head"] + test_2["sector"] + test_2["new_var_rest"]

test_2 = test_2.drop(columns=["new_var_name", "new_var_head", "new_var_rest", "sector"])

test_2 = test_2[["node", "variable", "unit", "year", "value"]]  # ordering the columns
...
test_full = test_full.sort_values(["node", "variable", "year"]).reset_index(drop=True)
...
FE_rep = FE_rep[["node", "variable", "unit", "year", "value"]].append(
    FE_rep_tot_fuel[["node", "variable", "unit", "year", "value"]], ignore_index=True
)
...
# ------------------------------------
# save these timeseries data

os.chdir(r"C:\Users\guof\Desktop\EFC")

FE_rep.to_csv("FE_rep.csv")
emiss_rep.to_csv("emiss_rep.csv")
sturm_rep.to_csv("sturm_rep.csv")

test_full.to_csv("sturm_rep_name_change.csv")

Fei = mp_ENE.scenario_list(model="MESSAGEix-GLOBIOM 1.1-BM-R12-EFC", default=False)
