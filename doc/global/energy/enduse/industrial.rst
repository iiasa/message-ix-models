...
Similar to the residential and commercial sectors,
the industrial sector in |name| distinguishes two demand categories,
thermal and specific.
Thermal demand, i.e., heat at different temperature levels, can be supplied by a variety of different energy carriers while specific demand requires electricity (or a decentralized technology to convert other energy carriers to electricity).
...
.. figure:: /_static/industry_end-use.png
   ...
   Schematic diagram of the industrial sector representation in |name|.

While cement production is not explicitly modeled at the process level in |name|,
the amount of cement production is linked to industrial activity
(more specifically, industrial thermal demand in MESSAGEix)
and the associated CO₂ emissions from the calcination process are accounted for explicitly.
In addition,
adding carbon capture and storage to mitigate these process-based CO₂ emission is available.
