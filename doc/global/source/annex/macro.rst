...
On the other hand, as a result of switching the implementation of `MESSAGE to GAMS <https://docs.messageix.org/en/stable/framework.html>`_, the iterative information exchange between the two models is now handled within GAMS. This accelerates the iteration process considerably, because the solution of the previous iteration is kept in memory and can serve as a starting point for the next iteration.
