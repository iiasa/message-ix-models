...
- These files are packaged, published, and installable from PyPI with :mod:`message_ix_models`
  —*unless* specifically excluded via :file:`MANIFEST.in` (see :ref:`large-input-data`, below).
...
- Documentation files like :file:`doc/pkg-data/*.rst` describe the contents of these files,
  and appear in the automatically-built documentation.
...
- Files in this directory are **private** and not installable from PyPI
  (because :mod:`message_data` is not packaged for or installable from PyPI).
- This is the preferred location for:
...
- Data files not distributable with :mod:`message_ix_models`,
  for instance those with access conditions (registration, payment, etc.).
...
and then use :meth:`.Context.get_local_path` and/or :func:`.local_data_path` to construct paths under this directory.

This setting can be made in multiple ways.
From lowest to highest precedence:

1. The default location is the *current working directory*:
   the directory in which the :program:`mix-models` :doc:`cli` is invoked,
...
4. The ``--local-data`` CLI option and related options
   such as the ``--output`` option to the ``report`` command.
...
Prefer text formats
   ...
   CSV files up to several thousand lines are compressed by Git automatically,
   and Git can handle diffs to these files easily.

*Do not* hard-code paths
   Data stored with (2–4) above can be retrieved with the utility functions mentioned,
   instead of hard-coded paths.

   For system-specific paths (4) only, get a :obj:`.Context` object
   and use it to get an appropriate :class:`~pathlib.Path` object pointing to a file:
...
Use a consistent scheme for data locations
   ...
   or (less preferable) alongside Python code files:
...
that is, any original sources and code to create the database used by :mod:`message_data`.
...
For the CLI, every command decorated with ``@click.pass_obj`` gets a first positional argument ``context``,
...
- Retrieve and respect configuration from existing objects.
  For instance, only duplicate settings with the same meaning when strictly necessary.
